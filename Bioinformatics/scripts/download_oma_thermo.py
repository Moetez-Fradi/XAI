...
from common_bio import download_cfg, raw_dir, write_json  # noqa: E402
...
def main() -> int:
    ...
    # Literature / manual expansion stub
    stub = out / "literature_pairs.stub.csv"
    if not stub.exists():
        stub.write_text(
            "uniprot_thermo,uniprot_meso,pdb_thermo,pdb_meso,note,citation\n"
    ...
    write_json(
        out / "download_manifest.json",
        {
            ...
            "errors": errors,
            "csv": str(csv_path),
            "meso_species": meso_code,
            "thermo_hint": thermo_hint,
        },
    )
    print(f"Thermo pairs: {len(rows)} written → {csv_path}")
    if not rows:
        print(
            "Note: 0 pairs matched filters; check OMA connectivity or edit seeds in download.yaml.",
            file=sys.stderr,
        )
    return 0
