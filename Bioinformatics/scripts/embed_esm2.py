...
def pick_device(pref: str) -> str:
    ...
    if pref and pref != 'auto':
        if pref == 'cuda':
            ...
            if not ok:
                print(f'WARN: --device cuda requested but GPU not usable ({reason}); using cpu', file=sys.stderr)
                return 'cpu'
        return pref
    ok, reason = cuda_usable()
    if ok:
    ...
    if torch.cuda.is_available() and (not ok):
        print(f'WARN: CUDA visible but not usable ({reason}); falling back to cpu', file=sys.stderr)
    return 'cpu'
...
def main() -> int:
    ...
    ap.add_argument('--split', choices=('train', 'holdout', 'all'), default='all', help='Which split to embed (default: all)')
    ap.add_argument('--limit', type=int, default=None, help='Max chains (default: all rows in the chosen corpus/split)')
    ap.add_argument('--batch-size', type=int, default=None)
    ap.add_argument('--device', default=None)
    ap.add_argument('--corpus-dir', type=Path, default=None, help='Override corpus directory containing chains.csv')
    ap.add_argument('--outdir', type=Path, default=None, help='Override embeddings output directory')
    ap.add_argument('--append', action='store_true', help='Append embeddings for corpus chains not already in outdir/ids.txt')
    ...
    ap.add_argument('--chains-file', type=Path, default=None, help='Embed only these chain_ids (one per line)')
    args = ap.parse_args()
    ...
    if not csv_path.exists():
        print(f"ERROR: missing {csv_path}. Run build_corpus.py{(' --smoke' if smoke else '')} first.", file=sys.stderr)
        return 1
    if args.outdir:
    ...
    if args.append:
        ...
        if not ids_path.exists() or not vec_path.exists():
            print(f'ERROR: --append requires existing {ids_path} and {vec_path}', file=sys.stderr)
            return 1
        existing_ids = [ln.strip() for ln in ids_path.read_text().splitlines() if ln.strip()]
        existing_mat = np.load(vec_path)
        if existing_mat.shape[0] != len(existing_ids):
            print(f'ERROR: vectors rows {existing_mat.shape[0]} != ids {len(existing_ids)}', file=sys.stderr)
            return 1
    ...
    print(f'Embedding {len(rows)} chains | device={device} batch={batch_size} max_len={max_len} layer={layer_spec} pooling={pooling} → {outdir}')
    import torch
    ...
    meta = {'model_id': esm_cfg.get('model_id'), 'local_dir': str(local_dir.relative_to(BIO_ROOT)), 'layer': str(layer_spec), 'pooling': pooling, 'max_seq_len': max_len, 'device': device, 'dim': int(mat.shape[1]), 'n': int(mat.shape[0]), 'split': args.split, 'corpus_dir': str(corpus_dir.relative_to(BIO_ROOT)), 'mode': 'smoke' if smoke else 'paper', 'dtype': 'float32'}
    write_json(outdir / 'meta.json', meta)
    write_json(outdir / 'embed_manifest.json', {**meta, 'elapsed_s': round(time.time() - t0, 1), 'vectors': str((outdir / 'vectors.npy').relative_to(BIO_ROOT)), 'ids': str((outdir / 'ids.txt').relative_to(BIO_ROOT))})
    if not np.isfinite(mat).all():
        print('ERROR: non-finite values in embeddings', file=sys.stderr)
        return 1
    norms = np.linalg.norm(mat, axis=1)
    print(f'Done: shape={mat.shape} | norm mean={norms.mean():.3f} std={norms.std():.3f} | {outdir}')
    return 0
