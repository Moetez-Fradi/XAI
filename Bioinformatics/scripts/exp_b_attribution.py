...
def pair_score(contrib: dict[str, float], M: np.ndarray, fq: np.ndarray, fh: np.ndarray, ranges: np.ndarray) -> float:
...
def main() -> int:
    ...
    if not exp_a_jsonl.exists() and (not exp_a_legacy.exists()):
        print(f'ERROR: missing Exp A results ({exp_a_jsonl} or {exp_a_legacy})', file=sys.stderr)
        return 1
    ...
    train_ids = [cid for cid, sp in splits.items() if sp == 'train' and cid in feats_map and (cid in id_to_row)]
    train_ids.sort()
    ...
    if len(train_ids) < 30:
        print(f'ERROR: only {len(train_ids)} train chains with features+embeddings; annotate more with annotate_dssp.py --from-exp-a --also-train N', file=sys.stderr)
        return 1
    ...
    np.savez_compressed(map_path, M=M, feature_names=np.array(feature_names), train_ids=np.array(train_ids), feature_ranges=ranges)
    top_dims = {}
    ...
    write_json(results_dir / 'dim_feature_map.json', {'n_train': len(train_ids), 'dim': int(M.shape[0]), 'features': feature_names, 'top_dims_per_feature': top_dims})
    run_config = {'mode': 'smoke' if smoke else 'paper', 'n_train_map': len(train_ids), 'features': feature_names, 'exp_a_jsonl': str(exp_a_jsonl.relative_to(BIO_ROOT)), 'features_table': str(feat_table.relative_to(BIO_ROOT)), 'embeddings': str(emb_dir.relative_to(BIO_ROOT)), 'max_neighbors_per_query': int(bcfg.get('max_neighbors_per_query', 5)), 'n_permutations': int(args.n_perm or bcfg.get('n_permutations', 1000)), 'permutation_seed': int(bcfg.get('permutation_seed', 42)), 'started_unix': int(time.time())}
    ...
    print(f'Exp B | train_map={len(train_ids)} planned_pairs={len(planned)} pending={len(pending)}')
    t0 = time.time()
    ...
    for i, (q, h, dims) in enumerate(pending):
        ...
        cos = float(np.dot(fq, fh) / (np.linalg.norm(fq) * np.linalg.norm(fh) + 1e-12))
        append_jsonl(pairs_jsonl, {'query': q, 'hit': h, 'pair_score': score, 'feature_cosine': cos, 'same_fold': False, 'n_dims': len(dims), 'attribution_mass': sum((abs(float(v)) for v in dims.values())), 'unix': int(time.time())})
    ...
    for b in range(n_perm):
        ...
        vals = [pair_score(dims, Mp, fq, fh, ranges) for dims, fq, fh in pair_payloads]
    ...
    for fi, name in enumerate(feature_names):
        ...
        for r in records:
            ...
            ys.append(float(1.0 - abs(feats_map[r['query']][fi] - feats_map[r['hit']][fi]) / (ranges[fi] if ranges[fi] > 1e-12 else 1.0)))
        per_feature[name] = {'pearson_attr_vs_agreement': pearson(np.array(xs), np.array(ys)), 'n': len(xs)}
    same = [r['pair_score'] for r in records if r.get('same_fold')]
    diff = [r['pair_score'] for r in records if not r.get('same_fold')]
    metrics = {'n_pairs': len(records), 'n_train_map': len(train_ids), 'mean_pair_score': real_mean, 'median_pair_score': real_median, 'null_mean': float(null_means.mean()), 'null_std': float(null_means.std()), 'null_ci_95': [float(np.quantile(null_means, 0.025)), float(np.quantile(null_means, 0.975))], 'p_value_right_tail': p_value, 'significant_0_05': p_value < 0.05, 'mean_score_same_fold': float(np.mean(same)) if same else None, 'mean_score_diff_fold': float(np.mean(diff)) if diff else None, 'n_same_fold': len(same), 'n_diff_fold': len(diff), 'per_feature': per_feature, 'n_permutations': n_perm, 'mode': 'smoke' if smoke else 'paper', 'elapsed_s': round(time.time() - t0, 1), 'completed_unix': int(time.time())}
    ...
    with (results_dir / 'summary.tsv').open('w') as f:
        ...
        for k in ('n_pairs', 'mean_pair_score', 'median_pair_score', 'null_mean', 'p_value_right_tail', 'significant_0_05', 'mean_score_same_fold', 'mean_score_diff_fold'):
            f.write(f'{k}\t{metrics[k]}\n')
        for name, st in per_feature.items():
            f.write(f"per_feature.{name}.pearson\t{st['pearson_attr_vs_agreement']}\n")
