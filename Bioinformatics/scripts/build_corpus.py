...
def _parse_chain_field(field: str) -> list[str]:
    """SCOPe chain field → author chain IDs (e.g. 'A:', 'A:1-100', 'A:,B:')."""
    chains: list[str] = []
    for part in field.split(","):
        ...
        if not head:
            continue
        # Multi-char rare; take first alphanumeric token
...
def load_scop_chain_map(cla_path: Path) -> dict[tuple[str, str], dict]:
    """Map (pdb_id, chain) → primary SCOPe annotation (first domain wins)."""
...
def load_sifts_uniprot(sifts_csv: Path) -> dict[tuple[str, str], str]:
    """Map (pdb, chain) → UniProt accession (first wins)."""
...
def extract_polymer_chains(path: Path) -> list[tuple[str, str]]:
    """Return list of (auth_chain_id, one_letter_seq) for polypeptides."""
    ...
    if path.suffix == ".gz" or path.name.endswith(".cif.gz"):
        ...
        # Fallback: PDB SEQRES via Bio.SeqIO is awkward; use PDBParser atoms
    ...
    if not (types and strands and seqs) or not (
        len(types) == len(strands) == len(seqs)
    ):
        return []
...
def stratified_holdout(
    ...
    *,
    ...
    # Take ~fraction from each fold (at least 0, round)
...
def main() -> int:
    ...
    ap.add_argument(
        "--smoke",
        action="store_true",
        help="Small subset → corpus_smoke/ (safe; does not touch paper lock)",
    )
    ap.add_argument(
        "--pilot",
        action="store_true",
        help="Alias for --smoke (compat with download scripts)",
    )
    ap.add_argument("--limit-pdbs", type=int, default=None)
    ap.add_argument(
        "--force-relock",
        action="store_true",
        help="Allow rewriting corpus_lock.json (dangerous for paper)",
    )
    args = ap.parse_args()
    ...
    if lock_path.exists() and not args.force_relock and not smoke:
        print(
            f"ERROR: {lock_path} already exists. "
            "Refusing to rebuild paper lock. Use --force-relock only if intentional.",
            file=sys.stderr,
        )
        return 1
    ...
    print(
        f"Building corpus mode={'smoke' if smoke else 'paper'} "
        f"pdbs={len(pdb_ids)} → {outdir}"
    )
            ...
            rows.append(
                {
                    ...
                    "uniprot": uniprot_map.get((pdb_id, chain), ""),
    ...
    holdout = stratified_holdout(
        chain_ids, folds, fraction=fraction, max_n=max_hold, seed=seed
    )
    ...
    lock = {
        ...
        "holdout_max": max_hold,
        "n_chains": len(rows),
        "n_train": len(train),
        "n_holdout": len(hold_list),
        "holdout_chain_ids": hold_list,
        "require_scop_fold": bool(cfg.get("require_scop_fold", True)),
        "min_len": int(cfg["min_len"]),
        "max_len": int(cfg["max_len"]),
        "pdb_ids_used": len(pdb_ids),
        "note": (
            "Held-out IDs are locked. Do not use holdout for tuning thresholds. "
            "Smoke locks are disposable; paper lock must not be rewritten."
        ),
    }
    write_json(lock_path, lock)

    manifest = {
        "outdir": str(outdir.relative_to(BIO_ROOT)),
        "mode": lock["mode"],
        "n_chains": len(rows),
        "n_train": len(train),
        "n_holdout": len(hold_list),
        ...
        "paths": {
            "chains_csv": str(csv_path.relative_to(BIO_ROOT)),
            "sequences_fasta": str(fasta.relative_to(BIO_ROOT)),
            "lock": str(lock_path.relative_to(BIO_ROOT)),
        },
    }
    write_json(outdir / "build_manifest.json", manifest)

    print(
        f"Corpus ready: {len(rows)} chains "
        f"(train={len(train)}, holdout={len(hold_list)}) → {outdir}"
    )
