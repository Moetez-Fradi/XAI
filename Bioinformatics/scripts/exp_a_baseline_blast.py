...
def which_or_die(name: str) -> str:
    ...
    if not p:
        raise FileNotFoundError(
            f"{name} not on PATH. source tools/env_linux.sh / micromamba bio-tools"
        )
    return p
...
def main() -> int:
    ...
    if not train_fa.exists() or not hold_fa.exists():
        print(
            "ERROR: missing FASTA. Run prep_exp_a_baselines.py first.",
            file=sys.stderr,
        )
        return 1
    ...
    if args.restart or not hits_path.exists():
        ...
        cmd = [
            ...
            str(db_prefix),
            "-outfmt",
            "6 qseqid sseqid evalue bitscore pident",
            "-evalue",
            str(blast_cfg.get("evalue", 10)),
            "-max_target_seqs",
            str(blast_cfg.get("max_target_seqs", 50)),
            "-num_threads",
            str(blast_cfg.get("num_threads", 4)),
            "-out",
            str(hits_path),
        ]
    ...
    # parse rankings (already sorted by blast per query typically by score)
    ...
    # ensure all holdout queries present
    hold_ids = []
