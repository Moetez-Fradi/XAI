...
from exp_b_v2_attribution import append_jsonl, contrib_vector, fit_dim_feature_map, global_feature_cosine, iter_exp_a, load_embeddings, load_features_table, load_jsonl, load_profiles, load_split, pair_scores, truncate_contrib
...
def load_or_fit_map(smoke: bool, bcfg: dict, ccfg: dict, emb_dir: Path, ann_dir: Path, splits: dict[str, str], id_to_row: dict[str, int], n_bins: int, n_profile_feat: int) -> tuple[np.ndarray, np.ndarray, list[str]]:
...
def sample_random_negatives(queries: list[str], query_folds: dict[str, str], train_by_fold: dict[str, list[str]], all_train: list[str], n_per_query: int, prng: np.random.Generator) -> list[tuple[str, str]]:
...
def score_pair_record(contrib: dict[str, float] | None, retrieval_score: float | None, q: str, h: str, profiles: dict[str, np.ndarray], global_map: dict[str, np.ndarray], m: np.ndarray, ranges: np.ndarray, dim: int, prng: np.random.Generator) -> dict:
...
def evaluate_method(records: list[dict], score_key: str, label_key: str='same_fold', subset: str | None=None) -> dict:
    rows = records
    if subset:
    ...
    s = np.array(scores)
    y = np.array(labels)
    return {'n': len(scores), 'n_positive': int(y.sum()), 'n_negative': int((~y).sum()), 'auroc': auroc(s, y), 'auprc': auprc(s, y), 'mean_score_positive': float(s[y].mean()) if y.any() else None, 'mean_score_negative': float(s[~y].mean()) if (~y).any() else None}
...
def main() -> int:
    ...
    results_dir = resolve_path(ccfg['smoke_results_dir'] if smoke else ccfg['results_dir'])
    ...
    exp_a_jsonl = resolve_path(ccfg['exp_a_smoke_jsonl'] if smoke else ccfg['exp_a_jsonl'])
    ...
    m, ranges, train_ids = load_or_fit_map(smoke, {}, ccfg, emb_dir, ann_dir, splits, id_to_row, n_bins, n_profile_feat)
    dim = int(m.shape[0])
    ...
    for rec in iter_exp_a(exp_a_jsonl, exp_a_legacy):
        ...
        for nb in (rec.get('neighbors') or [])[:max_neigh]:
            ...
            planned.append({'query': q, 'hit': h, 'pair_source': 'retrieval', 'same_fold': bool(q_fold and q_fold == h_fold), 'contrib': nb.get('dims_explained') or {}, 'retrieval_score': float(nb.get('score') or 0.0)})
    ...
    for q, h in sample_random_negatives(holdout_queries, query_folds, train_by_fold, train_chains, n_rand, prng):
        q_fold = query_folds.get(q, '')
        h_fold = query_folds.get(h, '')
        planned.append({'query': q, 'hit': h, 'pair_source': 'random_negative', 'same_fold': bool(q_fold and q_fold == h_fold), 'contrib': {}, 'retrieval_score': None})
    ...
    print(f'Exp C | map_train={len(train_ids)} planned={len(planned)} pending={len(pending)} (retrieval + random negatives)')
    t0 = time.time()
    ...
    for i, p in enumerate(pending):
        ...
        scores = score_pair_record(p.get('contrib') or None, rs, p['query'], p['hit'], profiles, global_map, m, ranges, dim, prng)
        gcos = scores.get('global_feature_cosine')
        saturated = not p.get('same_fold') and gcos is not None and (float(gcos) >= sat_min)
        row = {'query': p['query'], 'hit': p['hit'], 'pair_source': p['pair_source'], 'same_fold': bool(p.get('same_fold')), 'saturated_diff_fold': saturated, **scores, 'unix': int(time.time())}
    ...
    score_methods = ['xq_profile_spearman', 'retrieval_score', 'random_dims_spearman', 'uniform_dims_spearman', 'attribution_entropy', 'attribution_top1_mass']
    ...
    for method in ('xq_profile_spearman', 'retrieval_score'):
        ...
        sat_eval[method] = {'n': len(scores), 'mean': float(np.mean(scores)) if scores else None}
    ...
    sc = np.array([float(r[primary]) for r in records if r.get(primary) is not None and r[primary] == r[primary]])
    lab = np.array([bool(r.get('same_fold')) for r in records if r.get(primary) is not None and r[primary] == r[primary]])
    if sc.size > 10:
    ...
    for baseline in ('retrieval_score', 'random_dims_spearman'):
        sc2 = np.array([float(r[baseline]) for r in records if r.get(baseline) is not None and r[baseline] == r[baseline]])
        lab2 = np.array([bool(r.get('same_fold')) for r in records if r.get(baseline) is not None and r[baseline] == r[baseline]])
        if sc2.size > 10:
    ...
    metrics = {'version': 'exp_c', 'mode': 'smoke' if smoke else 'paper', 'n_pairs': len(records), 'n_retrieval': sum((1 for r in records if r.get('pair_source') == 'retrieval')), 'n_random_negative': sum((1 for r in records if r.get('pair_source') == 'random_negative')), 'n_same_fold': sum((1 for r in records if r.get('same_fold'))), 'n_train_map': len(train_ids), 'evaluations': evaluations, 'saturated_diff_fold': sat_eval, 'exp_b_v3_reference': {'mann_whitney_p': b3_ref.get('mann_whitney_p'), 'auroc_same_fold': b3_ref.get('auroc_same_fold'), 'label_perm_p': b3_ref.get('p_value_label_perm_fold_gap')}, 'elapsed_s': round(time.time() - t0, 1), 'completed_unix': int(time.time())}
    ...
    with (results_dir / 'summary.tsv').open('w') as f:
        ...
        for method, subs in evaluations.items():
            for sub, ev in subs.items():
                f.write(f"{method}\t{sub}\t{ev.get('n')}\t{ev.get('auroc')}\t{ev.get('auprc')}\t{ev.get('mean_score_positive')}\t{ev.get('mean_score_negative')}\n")
    ...
    print(json.dumps({'xq_profile_spearman_auroc': primary_auc.get('auroc'), 'retrieval_score_auroc': evaluations.get('retrieval_score', {}).get('all', {}).get('auroc'), 'random_dims_auroc': evaluations.get('random_dims_spearman', {}).get('all', {}).get('auroc')}, indent=2))
