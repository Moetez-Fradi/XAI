...
from exp_e_lib import _SITE_TYPES_DEFAULT, ensure_benchmark_dssp, load_dssp_residues, load_sifts_chain_table, load_sifts_chain_uniprot, load_uniprot_entry, map_sites_for_chain, normalize_feature_type
...
def main() -> int:
    ...
    max_q = int(ecfg.get('max_benchmark_queries_smoke' if smoke else 'max_benchmark_queries', 200))
    ...
    with chains_csv.open() as f:
        for rec in csv.DictReader(f):
            ...
            if require_ec_go and (not ecs) and (not gos):
                continue
            site_doc = map_sites_for_chain(cid, acc, entry, segs, [], site_types=site_types, catalytic_keywords=catalytic_kw, buffer=0)
            if site_doc['n_site_residues'] < min_sites:
                continue
            scored.append({'chain_id': cid, 'uniprot_accession': acc, 'scop_fold': rec.get('scop_fold', ''), 'length': length, 'n_site_residues_est': site_doc['n_site_residues'], 'ec_numbers': ecs, 'go_ids': gos, 'segs': segs, 'entry': entry})
    ...
    if not args.skip_dssp and dssp_targets:
        missing_dssp = {cid for cid in dssp_targets if len(load_dssp_residues(ann_dir, cid)) < min_len}
        if missing_dssp:
            print(f'  Backfilling DSSP for {len(missing_dssp)} holdout benchmark candidates...')
            ensure_benchmark_dssp(missing_dssp, ann_dir, str(ecfg.get('mkdssp_bin', 'auto')), chains_csv)
    holdout: list[dict] = []
    for row in scored:
        cid = row['chain_id']
        ...
        if len(residues) < min_len:
            continue
        site_doc = map_sites_for_chain(cid, row['uniprot_accession'], row['entry'], row['segs'], residues, site_types=site_types, catalytic_keywords=catalytic_kw, buffer=buffer)
        if site_doc['n_site_residues'] < min_sites:
            continue
        site_path = sites_dir / f'{cid}.json'
        if args.force or not site_path.exists():
            write_json(site_path, site_doc)
        holdout.append({'chain_id': cid, 'uniprot_accession': row['uniprot_accession'], 'scop_fold': row['scop_fold'], 'length': row['length'], 'n_site_residues': site_doc['n_site_residues'], 'ec_numbers': ';'.join(sorted(row['ec_numbers'])), 'go_ids': ';'.join(sorted(row['go_ids'])), 'site_json': str(site_path.relative_to(proc_dir))})
    ...
    with bench_tsv.open('w', newline='') as f:
        w = csv.DictWriter(f, fieldnames=['chain_id', 'uniprot_accession', 'scop_fold', 'length', 'n_site_residues', 'ec_numbers', 'go_ids', 'site_json'])
        w.writeheader()
        w.writerows(holdout)
    manifest = {'mode': 'smoke' if smoke else 'paper', 'n_benchmark_queries': len(holdout), 'n_scored_candidates': len(scored), 'n_dssp_backfill_targets': len(dssp_targets), 'require_ec_or_go': require_ec_go, 'min_site_residues': min_sites, 'benchmark_tsv': str(bench_tsv), 'sites_dir': str(sites_dir)}
