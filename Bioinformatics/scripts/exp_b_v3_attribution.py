...
def load_train_profiles_for_map(
    ann_dir: Path,
    splits: dict[str, str],
    ...
    """All train-split chains with binned profiles in annotations_dir."""
...
def attribution_diagnostics(a: np.ndarray) -> dict[str, float]:
    ...
    return {
        ...
        "frac_top1_above_0_9": float(np.mean(a.max(axis=1) >= 0.9)),
    }
...
def row_permutation_test(
    ...
    """Return (real_mean, p_value_right_tail, null_mean)."""
    y_ranks = rank_rows(y)
...
def label_permutation_gap(
    scores: np.ndarray, labels: np.ndarray, n_perm: int, seed: int
) -> tuple[float, float, float]:
    """Permute fold labels; test same_fold - diff_fold gap."""
...
def analyze_cohort_v3(
    ...
    real_all, p_global, null_global = row_permutation_test(
        sub_a, sub_y, m, n_perm, seed, mask=None
    )
    real_sf, p_sf, null_sf = row_permutation_test(
        sub_a, sub_y, m, n_perm, seed + 1, mask=sub_labels
    )
    ...
    return {
        ...
        "p_value_global_row_perm": p_global,
        "null_mean_global_row_perm": null_global,
        "p_value_same_fold_row_perm": p_sf,
        "null_mean_same_fold_row_perm": null_sf,
        "real_mean_same_fold_row_perm": real_sf,
        "p_value_label_perm_fold_gap": p_gap,
        "null_mean_label_perm_fold_gap": null_gap,
        "real_label_perm_fold_gap": gap,
        "significant_0_05_global_row_perm": p_global < 0.05,
        "significant_0_05_same_fold_row_perm": p_sf < 0.05,
        "significant_0_05_label_perm_gap": p_gap < 0.05,
        ...
        "n_same_fold": int(sub_labels.sum()),
        "n_diff_fold": int((~sub_labels).sum()),
        "auroc_same_fold": auc,
        "mann_whitney_p": mw_p,
        # legacy aliases for comparison tables
        "p_value_spearman": p_global,
        "null_mean_spearman": null_global,
        "significant_0_05_spearman": p_global < 0.05,
    }
...
def ablation_stats(
    ...
    y = np.stack(
        [profile_agreement(pq, ph, ranges) for _, pq, ph in pair_payloads]
    )
    y_ranks = rank_rows(y)
    scores = spearman_rows(a @ m, y, y_ranks)
    ...
    return {
        "mean_spearman": real,
        "p_value_spearman": p,
        "significant_0_05_spearman": p < 0.05,
        "attribution": attribution_diagnostics(a),
    }
...
def write_v2_comparison(path: Path, v2: dict, v3_primary: dict) -> None:
    rows = [
        ("metric", "v2", "v3"),
        ("n_pairs", v2.get("n_pairs"), v3_primary.get("n_pairs")),
        ("n_train_map", v2.get("n_train_map"), v3_primary.get("n_train_map")),
        ("mean_spearman", v2.get("primary_cohort", {}).get("mean_spearman"), v3_primary.get("mean_spearman")),
        (
            "p_global_row_perm",
            v2.get("primary_cohort", {}).get("p_value_spearman"),
            v3_primary.get("p_value_global_row_perm"),
        ),
        (
            "p_same_fold_row_perm",
            None,
            v3_primary.get("p_value_same_fold_row_perm"),
        ),
        (
            "p_label_perm_fold_gap",
            None,
            v3_primary.get("p_value_label_perm_fold_gap"),
        ),
        (
            "mean_spearman_same_fold",
            v2.get("primary_cohort", {}).get("mean_spearman_same_fold"),
            v3_primary.get("mean_spearman_same_fold"),
        ),
        (
            "mean_spearman_diff_fold",
            v2.get("primary_cohort", {}).get("mean_spearman_diff_fold"),
            v3_primary.get("mean_spearman_diff_fold"),
        ),
        ("mann_whitney_p", v2.get("primary_cohort", {}).get("mann_whitney_p"), v3_primary.get("mann_whitney_p")),
    ]
...
def main() -> int:
    ...
    # Pair profiles (Exp A chains only)
    needed: set[str] = set()
    ...
    if len(profiles_map) < 30:
        print(
            ...
            "Run: annotate_dssp.py --from-exp-a --store-profiles",
            file=sys.stderr,
        )
        return 1

    # Map training: all train profiles on disk (not limited to pair set)
    ...
    write_json(
        results_dir / "dim_profile_map.json",
        {
            ...
            "map_method": map_method,
            "ridge_alpha": ridge_alpha if map_method == "ridge" else None,
            "dim": int(m.shape[0]),
            "profile_dim": profile_dim,
            "max_abs_weight": float(np.abs(m).max()),
            "mean_abs_weight": float(np.abs(m).mean()),
        },
    )
    ...
    print(
        f"Exp B v3 | map={map_method} train={len(train_ids)} neighbors={max_neigh} "
    ...
    cohorts = [
        analyze_cohort_v3(
            records, a_full, y, m, labels, t, n_perm, seed
        )
    ...
    for n_top in bcfg.get("ablation_top_dims") or []:
        if top_n is not None and n_top != top_n:
            continue
        ablations[f"top_{n_top}"] = ablation_stats(
            pair_payloads, m, ranges, int(n_top), min(n_perm, 500), seed + int(n_top)
        )
    ...
    metrics = {
        ...
        "map_method": map_method,
        "max_neighbors_per_query": max_neigh,
        "profile_dim": profile_dim,
        "n_bins": n_bins,
        "top_dims_ablation": top_n,
        "attribution_diagnostics": attr_diag,
        "primary_cohort": primary,
        "primary_hypothesis": "same_fold_row_perm",
        "cohorts": cohorts,
        "ablations": ablations,
        "negative_control": {
        ...
        "v2_reference": {
            "path": str(v2_path.relative_to(BIO_ROOT)) if v2_path.exists() else None,
            "mean_spearman": v2.get("primary_cohort", {}).get("mean_spearman"),
            "p_value_global_row_perm": v2.get("primary_cohort", {}).get("p_value_spearman"),
            "n_pairs": v2.get("n_pairs"),
            "n_train_map": v2.get("n_train_map"),
        },
    ...
    print(
        json.dumps(
            {
                "primary": {
                    "same_fold_row_perm_p": primary.get("p_value_same_fold_row_perm"),
                    "label_perm_gap_p": primary.get("p_value_label_perm_fold_gap"),
                    "global_row_perm_p": primary.get("p_value_global_row_perm"),
                },
                "v2_reference": metrics["v2_reference"],
            },
            indent=2,
        )
    )
