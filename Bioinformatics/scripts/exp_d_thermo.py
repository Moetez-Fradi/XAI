...
from exp_d_stats import bootstrap_spearman_ci, perm_p_value_spearman  # noqa: E402
from exp_d_structure_features import (  # noqa: E402
    ensure_structure_features,
    load_cached,
    merge_structure_features,
)
from xqdrant_rest import XQdrantREST  # noqa: E402

# steps.md thermostability correlates (DSSP + structure)
THERMO_DELTA_KEYS = [
    "frac_charged_exposed",
    "mean_loop_length",
    "packing_density",
    "ion_pair_density",
]
...
def thermo_features(residues: list[dict]) -> dict:
    """Thermostability-related proxies from per-residue DSSP rows."""
    n = len(residues) or 1
    ...
    if run:
        coil_runs.append(run)
    return {
        "frac_charged": charged / n,
        "frac_charged_exposed": charged_exp / n,
        "frac_buried": buried / n,
        "mean_loop_length": float(np.mean(coil_runs)) if coil_runs else 0.0,
        "max_loop_length": float(max(coil_runs)) if coil_runs else 0.0,
        "packing_proxy": 1.0 - float(np.mean([float(r.get("rsa") or 0) for r in residues])) if residues else 0.0,
    }


def features_from_aggregates(agg: dict) -> dict:
    """Fallback when per-residue rows are not stored."""
    return {
        "frac_charged": None,
        "frac_charged_exposed": None,
        "frac_buried": 1.0 - float(agg.get("mean_rsa") or 0),
        "mean_loop_length": float(agg.get("frac_coil") or 0) * float(agg.get("n_residues") or 300),
        "max_loop_length": None,
        "packing_proxy": 1.0 - float(agg.get("mean_rsa") or 0),
        "frac_helix": float(agg.get("frac_helix") or 0),
        "frac_sheet": float(agg.get("frac_sheet") or 0),
        "frac_coil": float(agg.get("frac_coil") or 0),
        "mean_rsa": float(agg.get("mean_rsa") or 0),
    }
...
def ensure_dssp(chain_ids: set[str], ann_dir: Path, mkdssp_cfg: str, corpus_csv: Path) -> None:
    """Annotate missing chains with per-residue DSSP (needed for thermo proxies)."""
    from annotate_dssp import (  # noqa: E402
        aggregate,
        chain_b_factors,
        find_mkdssp,
        find_structure,
        ...
        try:
            with tempfile.TemporaryDirectory(prefix="dssp_d_") as td:
                ...
                if not residues:
                    ...
                    subprocess.run(
                        [mk, str(struct_path), str(dssp_out)],
                        check=True,
                        capture_output=True,
                        text=True,
                    )
...
def main() -> int:
    ...
    from prep_exp_d_pairs import resolve_corpus_dir  # noqa: E402

    corpus_dir = resolve_corpus_dir(dcfg, corpus_cfg, smoke)

    results_dir = resolve_path(
        dcfg["smoke_results_dir"] if smoke else dcfg["results_dir"]
    )
    ...
    ensure_dssp(
        chain_ids,
        ann_dir,
        resolve_path(dcfg.get("mkdssp_bin", "tools/mamba/envs/bio-tools/bin/mkdssp")),
        corpus_dir / "chains.csv",
    )
    ensure_structure_features(
        chain_ids,
        corpus_dir / "chains.csv",
    ...
    for i, p in enumerate(pending):
        ...
        hits = client.query(
            ...
            limit=top_k,
            with_payload=True,
            with_dims_explained={"top": dims_top},
        ...
        # Direct pair attribution: query filtered to thermo hit (for case-study pairs)
        if dims is None and thermo in id_to_row:
            direct = client.query(
                ...
                with_payload=True,
                with_dims_explained={"top": dims_top},
                query_filter={"must": [{"key": "chain_id", "match": {"value": thermo}}]},
            )
            if direct:
                h0 = direct[0]
        ...
        delta = {
            k: float(ft[k]) - float(fm[k])
            for k in fm
            if fm.get(k) is not None and ft.get(k) is not None
        }
    ...
    # Delta feature means across pairs (steps.md thermostability correlates)
    delta_keys = list(THERMO_DELTA_KEYS)
    delta_means = {}
    for k in delta_keys:
        vals = [
            float(r["features_delta"][k])
            for r in records
            if r.get("features_delta") and r["features_delta"].get(k) is not None
        ]
    ...
    for src, rs in sorted(by_source.items()):
        ...
        source_metrics[src] = {
            "n_pairs": len(rs),
            "recovery_rate": sum(1 for x in rs if x.get("thermo_in_topk")) / len(rs) if rs else 0.0,
            "mean_xq_profile_spearman": float(np.mean(src_scores)) if src_scores else None,
        }

    # Correlation: attribution profile score vs thermostability proxy deltas + significance
    corr_pairs = [
        r
        for r in records
        if r.get("xq_profile_spearman") is not None and r.get("features_delta")
    ]
    attr_vs_delta = {}
    attr_vs_delta_p = {}
    attr_vs_delta_ci = {}
    ...
    for ki, k in enumerate(delta_keys):
        xs = np.array(
            [float(r["xq_profile_spearman"]) for r in corr_pairs if r["features_delta"].get(k) is not None],
            dtype=np.float64,
        )
        ys = np.array(
            [float(r["features_delta"][k]) for r in corr_pairs if r["features_delta"].get(k) is not None],
            dtype=np.float64,
        )
        if len(xs) >= 3:
    ...
    metrics = {
        ...
        "n_pairs": n,
        "ortholog_recovery_topk": {
            "top_k": top_k,
            "n_recovered": recovered,
            "rate": recovered / n if n else 0.0,
        },
        "mean_xq_profile_spearman": float(np.mean(scores)) if scores else None,
        "mean_feature_delta": delta_means,
        "by_source": source_metrics,
        "attribution_vs_delta_spearman": attr_vs_delta,
        "attribution_vs_delta_p_value": attr_vs_delta_p,
        "attribution_vs_delta_ci95": attr_vs_delta_ci,
        "attribution_vs_delta_n": attr_vs_delta_n,
        "n_permutations": n_perm,
        "n_bootstrap": n_boot,
        "permutation_seed": seed,
        "thermo_delta_keys": delta_keys,
        "ion_pair_cutoff_angstrom": ion_cutoff,
    ...
    with (results_dir / "significance.tsv").open("w") as f:
        ...
        for k in delta_keys:
            ...
            f.write(
                f"{k}\t{attr_vs_delta_n.get(k, '')}\t{attr_vs_delta[k]}\t{p}\t"
                f"{ci.get('lo', '')}\t{ci.get('hi', '')}\t{sig}\n"
            )
    ...
    with (results_dir / "pymol_pairs.tsv").open("w") as f:
        f.write(
            "meso_chain\tthermo_chain\tthermo_rank\txq_spearman\t"
            "delta_charged_exp\tdelta_packing\tdelta_ion_pairs\tdelta_loop_len\n"
        )
        for r in records:
            fd = r.get("features_delta") or {}
            f.write(
                f"{r['meso_chain_id']}\t{r['thermo_chain_id']}\t{r.get('thermo_rank')}\t"
                f"{r.get('xq_profile_spearman')}\t{fd.get('frac_charged_exposed')}\t"
                f"{fd.get('packing_density')}\t{fd.get('ion_pair_density')}\t"
                f"{fd.get('mean_loop_length')}\n"
            )
