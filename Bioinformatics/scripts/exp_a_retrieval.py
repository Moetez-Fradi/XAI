...
def hit_rates(flags: list[list[bool]], ks: list[int]) -> dict[str, float]:
    ...
    for k in ks:
        ...
        prec = [
            (sum(row[:k]) / len(row[:k])) if row[:k] else 0.0 for row in flags
        ]
...
def bootstrap_cis(
    ...
    *,
    ...
    """Percentile CIs for recall@k by resampling queries."""
...
def record_to_flags(rec: dict) -> tuple[list[bool], list[bool], list[bool]]:
    ...
    return (
        [bool(n.get("same_fold")) for n in neigh],
        [bool(n.get("same_superfamily")) for n in neigh],
        [bool(n.get("same_family")) for n in neigh],
    )
...
def finalize_metrics(
    ...
    *,
    ...
    return {
        ...
        "rank_mismatches_plain_vs_explained": rank_mismatches,
        "queries_with_dims_explained": explain_present,
        "attribution_rank_stable": rank_mismatches == 0,
        "elapsed_s": round(elapsed_s, 1),
        "collection": coll,
        "mode": "smoke" if smoke else "paper",
        "dims_explained_top": dims_top,
...
def write_summary_tsv(path: Path, metrics: dict) -> None:
    with path.open("w") as f:
        ...
        f.write(
            f"meta\tattribution_rank_stable\t{int(metrics['attribution_rank_stable'])}\n"
        )
...
def main() -> int:
    ...
    ap.add_argument(
        "--restart",
        action="store_true",
        help="Wipe results/checkpoints for this run and start over",
    )
    ...
    queries = [
        ...
        if m.get("split") == query_split and cid in id_to_row
    ]
    queries.sort()
    max_q = args.max_queries
    if max_q is None:
        max_q = (
            int(exp_cfg.get("max_queries_smoke", 20))
            if smoke
            else int(exp_cfg.get("max_queries_paper", 0))
        )
    ...
    if not queries:
        print(
            f"ERROR: no queries with split={query_split} in embeddings.",
            file=sys.stderr,
        )
        return 1
    ...
    run_config = {
        ...
        "index_filter_split": filter_split,
        "topk": topk,
        "limits": ks,
        "dims_explained_top": dims_top,
        ...
        "corpus_lock": str(lock_path.relative_to(BIO_ROOT)) if lock_path.exists() else None,
        "index_manifest": (
            str(index_manifest.relative_to(BIO_ROOT)) if index_manifest.exists() else None
        ),
    ...
    if pending:
        ...
        if not client.collection_exists(coll):
            print(
                f"ERROR: collection {coll} missing. Run index_xqdrant.py"
    ...
    print(
        f"Exp A | collection={coll} queries={len(queries)} "
        f"(split={query_split}) filter={filter_split} topk={topk}"
    )
    ...
    for qi, qid in enumerate(pending):
        ...
        plain = client.query(
            ...
            limit=topk,
            with_payload=True,
            with_dims_explained=None,
            query_filter=qfilter,
        )
        explained = client.query(
            coll,
            qvec,
            limit=topk,
            with_payload=True,
        ...
        expl_ids = [
            str(h.get("payload", {}).get("chain_id", h["id"])) for h in explained
        ]
            ...
            neigh_rows.append(
                {
                    ...
                    "score": h.get("score"),
                    "score_with_explain": (
                        explained[rank].get("score") if rank < len(explained) else None
                    ),
                    "scop_fold": pl.get("scop_fold"),
                    "scop_sccs": pl.get("scop_sccs"),
                    "scop_class": pl.get("scop_class"),
                    "uniprot": pl.get("uniprot"),
                    "same_fold": same_f,
                    "same_family": same_fam,
                    "same_superfamily": same_sf,
                    "dims_explained": dims,
                }
            )
        ...
        rec = {
            "query": qid,
            "query_point_id": id_to_row[qid],
            "pdb_id": qmeta.get("pdb_id"),
            "chain": qmeta.get("chain"),
            "length": qmeta.get("length"),
            "uniprot": qmeta.get("uniprot"),
            "scop_domain": qmeta.get("scop_domain"),
            "scop_fold": qfold,
            "scop_sccs": qsccs,
            "scop_class": qmeta.get("scop_class"),
            "plain_neighbor_ids": plain_ids,
            "explained_neighbor_ids": expl_ids,
            "plain_scores": plain_scores,
            "explained_scores": expl_scores,
            "neighbors": neigh_rows,
            "rank_match_with_explain": plain_ids == expl_ids,
            "elapsed_s": round(time.time() - t_q, 4),
            "unix": int(time.time()),
        }
        ...
        # Partial metrics every 25 queries (and last)
        if len(done_records) % 25 == 0 or qi + 1 >= len(pending):
            partial = finalize_metrics(
                done_records,
                ks=ks,
                coll=coll,
                smoke=smoke,
    ...
    # Ensure records follow planned query order for final dumps
    ...
    write_json(
        state_path,
        {
            "n_done": len(ordered),
            "n_planned": len(queries),
            "complete": len(ordered) == len(queries),
            "updated_unix": int(time.time()),
        },
    )
    ...
    if metrics["rank_mismatches_plain_vs_explained"]:
        print(
            f"WARN: {metrics['rank_mismatches_plain_vs_explained']} queries "
            "had rank changes with dims_explained",
            file=sys.stderr,
        )
        return 2
    if metrics["queries_with_dims_explained"] == 0:
        print(
            "ERROR: no dims_explained in responses — is this the XQdrant fork?",
            file=sys.stderr,
        )
        return 1
    return 0
