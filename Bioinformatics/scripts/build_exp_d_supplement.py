...
from build_corpus import _find_cla, extract_polymer_chains, find_structure, load_scop_chain_map, load_sifts_uniprot, pass_filters
...
def main() -> int:
    ...
    if not ids_file.exists():
        ...
        return 1
    pdb_ids = [ln.strip().lower() for ln in ids_file.read_text().splitlines() if ln.strip() and (not ln.startswith('#'))]
    if not pdb_ids:
    ...
    for i, pdb_id in enumerate(pdb_ids, 1):
        ...
        for chain, seq in polymers:
            ...
            if filter_u and u not in target_u:
                ...
                continue
            rows.append({'chain_id': f'{pdb_id}_{chain}', 'pdb_id': pdb_id, 'chain': chain, 'length': len(seq), 'split': 'train', 'scop_domain': (scop or {}).get('scop_domain', ''), 'scop_sccs': (scop or {}).get('scop_sccs', ''), 'scop_fold': (scop or {}).get('scop_fold', ''), 'scop_class': (scop or {}).get('scop_class', ''), 'uniprot': u, 'structure_file': str(path.relative_to(BIO_ROOT)), 'sequence': seq})
            kept += 1
    ...
    with fasta.open('w') as f:
        for r in rows:
            ...
            for j in range(0, len(seq), 80):
                f.write(seq[j:j + 80] + '\n')
    write_json(outdir / 'build_manifest.json', {'source': 'exp_d_supplement', 'n_chains': len(rows), 'n_pdbs_requested': len(pdb_ids), 'n_target_uniprots': len(target_u), 'skipped': skipped, 'elapsed_s': round(time.time() - t0, 1), 'note': 'All supplement chains are train split; paper holdout lock unchanged.'})
