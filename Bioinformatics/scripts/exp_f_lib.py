...
from exp_e_lib import which_tmalign  # noqa: E402
...
def which_or_die(name: str) -> str:
    ...
    candidates = [
        BIO_ROOT / "tools/blast/bin" / name,
        BIO_ROOT / "tools/mamba/envs/bio-tools/bin" / name,
    ]
    for p in candidates:
...
class ESM2SingleEmbedder:
    """Load ESM2 once; time per-sequence mean-pool embeddings."""

    def __init__(self, *, model_dir: Path, device: str = "auto", max_len: int = 1024) -> None:
        import torch
        from transformers import AutoModel, AutoTokenizer

        from embed_esm2 import cuda_usable, mean_pool, pick_device  # noqa: E402
...
def time_tmalign_candidates(
    *,
    ...
    """Return (elapsed_s, n_alignments_run)."""
    t0 = time.perf_counter()
    n = 0
    for hid in candidates[:max_candidates]:
        ...
        if not t_pdb.exists():
            continue
        subprocess.run(
            [tmalign, str(query_pdb), str(t_pdb)],
            capture_output=True,
            text=True,
            check=False,
        )
...
def summarize_times(values: list[float]) -> dict[str, float | int | None]:
    ...
    return {
        ...
        "p25_s": round(float(np.percentile(arr, 25)), 3),
        "p75_s": round(float(np.percentile(arr, 75)), 3),
