...
def pick_meso_chains(rows: list[dict], uniprots: set[str], *, explicit: list[str], max_meso: int, min_len: int, prefer_holdout: bool) -> list[dict]:
    if explicit:
        by_id = {r['chain_id']: r for r in rows}
        out = [by_id[cid] for cid in explicit if cid in by_id]
        return out[:max_meso]
    cands = [r for r in rows if r.get('uniprot') in uniprots and int(r.get('length') or 0) >= min_len]
    cands.sort(key=lambda r: (r['split'] != 'holdout' if prefer_holdout else False, r['chain'] not in {'A', 'B'}, r['chain_id']))
...
def pick_thermo_reps(rows: list[dict], uniprots: set[str], thermo_pdb_prefixes: set[str], *, min_len: int, one_per_pdb: bool) -> list[dict]:
    ...
    cands = [r for r in rows if is_thermo(r) and r.get('chain') == 'A' and (int(r.get('length') or 0) >= min_len)]
    if not cands:
...
def main() -> int:
    ...
    for grp in dcfg.get('ortholog_groups') or []:
        ...
        meso_chains = pick_meso_chains(rows, meso_u, explicit=list(grp.get('meso_chain_ids') or []), max_meso=int(grp.get('max_meso_chains', 3)), min_len=min_len, prefer_holdout=bool(grp.get('prefer_holdout', True)))
        thermo_chains = pick_thermo_reps(rows, thermo_g, thermo_pdb, min_len=min_len, one_per_pdb=bool(grp.get('one_thermo_per_pdb', True)))
        n = 0
        for m in meso_chains:
            for t in thermo_chains:
                if m['chain_id'] == t['chain_id']:
                    continue
                lit_rows.append({'uniprot_thermo': t.get('uniprot', ''), 'uniprot_meso': m.get('uniprot', ''), 'pdb_thermo': t['chain_id'], 'pdb_meso': m['chain_id'], 'note': grp.get('name', ''), 'citation': grp.get('citation', '')})
                n += 1
        group_stats.append({'name': grp.get('name'), 'meso_chains': len(meso_chains), 'thermo_reps': len(thermo_chains), 'pairs': n})
    ...
    if stub.exists():
        with stub.open() as f:
            for row in csv.DictReader(f):
                ...
                if meso and thermo:
                    lit_rows.append(row)
    fields = ['uniprot_thermo', 'uniprot_meso', 'pdb_thermo', 'pdb_meso', 'note', 'citation']
    with out_path.open('w', newline='') as f:
        w = csv.DictWriter(f, fieldnames=fields, extrasaction='ignore')
        w.writeheader()
        w.writerows(lit_rows)
    write_json(out_path.parent / 'literature_manifest.json', {'n_pairs': len(lit_rows), 'groups': group_stats, 'csv': str(out_path)})
