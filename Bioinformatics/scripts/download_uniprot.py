...
def _load_pdb_ids(pdb_dir: Path) -> set[str]:
    ...
    raise FileNotFoundError(
        f"Missing paper_ids.txt / pilot_ids.txt in {pdb_dir}. Run download_pdb.py first."
    )
...
def _batch_fetch(
    ...
    *,
    ...
    """Fetch UniProt entries via GET /uniprotkb/accessions?accessions=..."""
    ensure_dir(dest_dir)
    ok, fail = 0, []
    pending = [
        a
        for a in accessions
        if force
        or not (dest_dir / f"{a}.json").exists()
        or (dest_dir / f"{a}.json").stat().st_size == 0
    ]
    ...
    for i in range(0, len(pending), batch_size):
        ...
        url = (
            ...
            f"?accessions={qs}&format=json&size={len(chunk)}"
        )
        req = Request(
            url,
            headers={
                "Accept": "application/json",
                "User-Agent": "XQdrant-Bioinformatics/0.1",
            },
        )
        try:
...
def main() -> int:
    ...
    ap.add_argument(
        "--sifts-only",
        action="store_true",
        help="Only SIFTS + Swiss-Prot bulk files; skip per-accession JSON",
    )
    ap.add_argument(
        "--skip-sprot",
        action="store_true",
        help="Skip large Swiss-Prot .dat/.fasta downloads",
    )
    args = ap.parse_args()
    ...
    # --- SIFTS maps (uniprot, GO, EC) ---
    sifts_files = []
    for item in cfg.get("sifts") or [
        {"name": "pdb_chain_uniprot.csv.gz", "url": cfg.get("sifts_url")}
    ]:
        if not item.get("url"):
            continue
    ...
    if not args.skip_sprot:
        ...
        if cfg.get("sprot_fasta_gz"):
            download_file(
                cfg["sprot_fasta_gz"],
                uni_dir / "uniprot_sprot.fasta.gz",
                force=args.force,
                timeout=600,
            )
    ...
    if args.sifts_only:
        write_json(
            uni_dir / "download_manifest.json",
            {"mode": mode, "sifts_only": True, "sifts": sifts_files},
        )
    ...
    ok, fail = _batch_fetch(
        accessions,
        json_dir,
        args.force,
        batch_size=int(cfg.get("batch_size", 100)),
        sleep_s=float(cfg.get("sleep_s", 0.15)),
    )

    write_json(
        uni_dir / "download_manifest.json",
        {
            "source": "UniProt REST + SIFTS + Swiss-Prot FTP",
            "mode": mode,
            "sifts": sifts_files,
