...
# Max ASA reference (Tien et al. 2013-ish / common DSSP RSA denominators)
MAX_ASA = {
    "A": 129.0,
    "R": 274.0,
    "N": 195.0,
...
def chain_b_factors(structure_path: Path, chain_id: str) -> dict[int, float]:
    """Mean backbone B-factor per residue seq id (auth)."""
    ...
    for res in chain:
        ...
        vals = [
            atom.get_bfactor()
            for atom in res
            if atom.element != "H" and atom.get_name() in {"CA", "N", "C", "O"}
        ]
        if not vals:
...
def run_dssp_biopython(structure_path: Path, mkdssp: str, chain_id: str) -> list[dict]:
    ...
    for key, val in dssp.property_dict.items():
        ...
        if str(ch).upper() != chain_id.upper():
            continue
        # BioPython DSSP tuple: (index, aa, ss, rel_asa, phi, psi, ...)
        aa = val[1]
        ss = val[2]
        try:
            rsa = float(val[3])
...
def parse_dssp_file(dssp_path: Path, chain_id: str) -> list[dict]:
    """Fallback classic DSSP parser if BioPython DSSP fails."""
    rows: list[dict] = []
    started = False
    with dssp_path.open() as f:
        for line in f:
            ...
            if not started or len(line) < 35:
                continue
            # Fixed-width-ish: chain often col 11, aa col 13, ss col 16, acc 35-38
            ch = line[11].strip() if len(line) > 11 else ""
            if ch.upper() != chain_id.upper():
                # also try whitespace form from mkdssp 4
                parts = line.split()
                if len(parts) < 6:
                    continue
                # heuristic: find single-letter chain then AA
                continue
            aa = line[13].strip()
            ss = line[16].strip() or "-"
            try:
            ...
            rows.append(
                {
                    ...
                    "aa": aa,
                    "ss": ss,
                    "rsa": min(acc / max_asa, 1.5) if max_asa else 0.0,
                    "acc": acc,
                }
            )
    return rows
...
def aggregate(residues: list[dict], bfactors: dict[int, float]) -> dict:
    ...
    return {
        ...
        "frac_sheet": sheet / n,
        "frac_coil": coil / n,
        "mean_hydrophobicity": float(sum(hydros) / len(hydros)) if hydros else 0.0,
        "mean_rsa": float(sum(rsas) / len(rsas)) if rsas else 0.0,
        "mean_b_factor": float(sum(bfs) / len(bfs)) if bfs else 0.0,
    }
...
def main() -> int:
    ...
    ap.add_argument(
        ...
        type=int,
        default=None,
        help="Also annotate N train chains for dim-feature map (with --from-exp-a)",
    )
    ap.add_argument(
        "--store-residues",
        action="store_true",
        ...
        try:
            with tempfile.TemporaryDirectory(prefix="dssp_") as td:
                ...
                if not residues:
                    ...
                    subprocess.run(
                        [mkdssp, str(struct_path), str(dssp_out)],
                        check=True,
                        capture_output=True,
                        text=True,
                    )
                ...
                payload = {
                    ...
                    "structure_file": str(src.relative_to(BIO_ROOT)),
                    "features": agg,
                    "n_residues_dssp": len(residues),
                }
    ...
    # Rebuild features CSV from all JSONs
    ...
    with features_csv.open("w", newline="") as f:
        ...
        for jp in json_files:
            ...
            row = {
                ...
                "scop_fold": data.get("scop_fold", ""),
                "n_residues": feats.get("n_residues", data.get("n_residues_dssp", 0)),
            }
    ...
    write_json(
        ckpt_path,
        {
            ...
            "ok_this_run": ok,
            "fail_this_run": fail,
            "features_table": str(features_csv.relative_to(BIO_ROOT)),
            "elapsed_s": round(time.time() - t0, 1),
            "updated_unix": int(time.time()),
        },
    )
    print(
        f"DSSP done: wrote {len(json_files)} chain JSONs, table={features_csv} "
        f"(ok={ok} fail={fail} this run)"
    )
