...
def main() -> int:
    ...
    results_root = resolve_path(bcfg['smoke_results_dir'] if smoke else bcfg['results_dir'])
    foldseek_rankings = results_root / 'foldseek' / 'rankings.json'
    if not foldseek_rankings.exists():
        print('ERROR: need Foldseek rankings first (exp_a_baseline_foldseek.py).', file=sys.stderr)
        return 1
    ...
    print(f'TM-align re-rank: {len(pending)} pending / {len(fs_ranks)} queries (candidates≤{max_cand}, threads={threads})')
    ...
    metrics['note'] = 'TM-align re-ranks Foldseek candidates (not exhaustive gallery search).'
