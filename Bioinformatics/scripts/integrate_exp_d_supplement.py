...
from common_bio import BIO_ROOT, ensure_dir, load_yaml, resolve_path, write_json  # noqa: E402
...
def merge_corpora(main_csv: Path, supp_csv: Path, out_dir: Path) -> tuple[int, int]:
    ...
    for r in supp_rows:
        cid = r["chain_id"]
        if cid in merged:
            # Main corpus wins for existing chains (preserve holdout splits).
            continue
        merged[cid] = r
        n_new += 1
    ...
    write_json(
        ...
        {
            "main_chains": len(main_rows),
            "supplement_chains": len(supp_rows),
            "merged_chains": len(rows),
            "new_from_supplement": n_new,
            "n_train": len(train),
            "n_holdout": len(holdout),
            "note": "Holdout IDs preserved from main corpus; supplement adds train only.",
        },
    )
...
def main() -> int:
    ...
    if not args.skip_embed:
        cmd = [
            str(py),
            str(BIO_ROOT / "scripts" / "embed_esm2.py"),
            "--append",
            "--corpus-dir",
            str(merged_dir),
            "--outdir",
    ...
    if not args.skip_index:
        cmd = [
            str(py),
            str(BIO_ROOT / "scripts" / "index_xqdrant.py"),
            "--corpus-dir",
            str(merged_dir),
        ]
