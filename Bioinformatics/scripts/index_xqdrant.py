...
def load_payload_map(chains_csv: Path) -> dict[str, dict]:
    ...
    with chains_csv.open() as f:
        for rec in csv.DictReader(f):
            out[rec['chain_id']] = {'chain_id': rec['chain_id'], 'pdb_id': rec['pdb_id'], 'chain': rec['chain'], 'split': rec['split'], 'scop_domain': rec.get('scop_domain', ''), 'scop_sccs': rec.get('scop_sccs', ''), 'scop_fold': rec.get('scop_fold', ''), 'scop_class': rec.get('scop_class', ''), 'uniprot': rec.get('uniprot', ''), 'length': int(rec['length']) if rec.get('length') else 0}
    return out

def file_sha256(path: Path, *, max_bytes: int=8000000) -> str:
    h = hashlib.sha256()
    ...
    with path.open('rb') as f:
        if size <= max_bytes:
            ...
            h.update(f.read(1000000))
            f.seek(max(size - 1000000, 0))
            h.update(f.read(1000000))
    return h.hexdigest()
...
def main() -> int:
    ...
    ap.add_argument('--recreate', '--restart', action='store_true', dest='restart', help='Drop collection + clear checkpoint and start over')
    ap.add_argument('--batch-size', type=int, default=None)
    ap.add_argument('--url', default=None)
    ap.add_argument('--collection', default=None)
    ap.add_argument('--limit', type=int, default=None, help='Upsert only first N (debug)')
    ap.add_argument('--corpus-dir', type=Path, default=None, help='Override corpus directory for payload metadata (e.g. corpus_merged)')
    args = ap.parse_args()
    ...
    if vectors.ndim != 2 or vectors.shape[0] != len(ids):
        print(f'ERROR: vectors shape {vectors.shape} vs {len(ids)} ids', file=sys.stderr)
        return 1
    ...
    if missing_payload:
        print(f'WARN: {missing_payload} embedding ids missing from chains.csv (payload will be minimal)', file=sys.stderr)
    ...
    try:
        ...
        print(f'ERROR: {e}\nStart the server first: ./start_xqdrant.sh', file=sys.stderr)
        return 1
    ...
    def write_manifest(*, n_collection: int, elapsed_s: float, upserted_now: int) -> None:
        write_json(manifest_path, {'url': url, 'collection': coll, 'mode': 'smoke' if smoke else 'paper', 'n_upserted': len(ids), 'n_upserted_this_run': upserted_now, 'n_in_collection': n_collection, 'dim': dim, 'distance': xcfg.get('distance', 'Cosine'), 'embeddings': emb_rel, 'embeddings_meta': emb_meta, 'embeddings_vectors_fingerprint': file_sha256(vec_path), 'corpus': str(corpus_dir.relative_to(BIO_ROOT)), 'corpus_lock': str(lock_path.relative_to(BIO_ROOT)) if lock_path.exists() else None, 'id_map': str(id_map_path.relative_to(BIO_ROOT)), 'checkpoint': str(ckpt_path.relative_to(BIO_ROOT)), 'elapsed_s': round(elapsed_s, 1), 'point_id_scheme': 'row index in embeddings/*/ids.txt (0-based)', 'completed_unix': int(time.time())})
    if not client.collection_exists(coll):
        print(f"Creating collection {coll} size={dim} distance={xcfg.get('distance', 'Cosine')}")
        client.create_collection(coll, size=dim, distance=str(xcfg.get('distance', 'Cosine')))
        start_idx = 0
    else:
        ...
        if not args.restart and n_existing == len(ids):
            write_id_map()
            write_json(ckpt_path, {'collection': coll, 'embeddings': emb_rel, 'n_total': len(ids), 'next_index': len(ids), 'updated_unix': int(time.time())})
            write_manifest(n_collection=n_existing, elapsed_s=0.0, upserted_now=0)
            print(f'Already complete: collection_count={n_existing} == n_ids={len(ids)}')
        ...
        if ckpt_path.exists() and (not args.restart):
            ...
            if ckpt.get('n_total') == len(ids) and ckpt.get('embeddings') == emb_rel:
                ...
                print(f'Resume checkpoint: next_index={start_idx}/{len(ids)} (collection count={n_existing})')
            else:
                print('WARN: checkpoint mismatch (corpus/size changed); re-upserting from 0 (idempotent ids)', file=sys.stderr)
        else:
            print(f'Collection {coll} exists ({n_existing} points) — upserting from 0 (idempotent)')
    write_id_map()
    ...
    for start in range(start_idx, len(ids), batch_size):
        ...
        for i in range(start, end):
            cid = ids[i]
            pl = payload_map.get(cid, {'chain_id': cid, 'split': '', 'scop_fold': ''})
            points.append({'id': i, 'vector': vectors[i].astype(np.float32).tolist(), 'payload': pl})
        client.upsert(coll, points, wait=True)
        write_json(ckpt_path, {'collection': coll, 'embeddings': emb_rel, 'n_total': len(ids), 'next_index': end, 'updated_unix': int(time.time())})
        for _ in range(end - start):
            progress.tick(ok=True, force_print=end >= len(ids))
    n = client.count(coll)
    write_manifest(n_collection=n, elapsed_s=time.time() - t0, upserted_now=len(ids) - start_idx)
