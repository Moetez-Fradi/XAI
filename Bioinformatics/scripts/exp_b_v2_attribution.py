...
def profile_agreement(pq: np.ndarray, ph: np.ndarray, ranges: np.ndarray) -> np.ndarray:
...
def contrib_vector(contrib: dict[str, float], dim: int, top_n: int | None=None) -> np.ndarray:
...
def pair_scores(contrib: dict[str, float], M: np.ndarray, pq: np.ndarray, ph: np.ndarray, ranges: np.ndarray, top_n: int | None=None) -> tuple[float, float]:
...
def load_profiles(ann_dir: Path, chain_ids: set[str], n_bins: int, n_profile_feat: int) -> dict[str, np.ndarray]:
    ...
    if missing and len(missing) <= 5:
        ...
        print(f'  WARN: {len(missing)} chains missing binned profiles (run annotate_dssp.py --store-profiles)', file=sys.stderr)
    return out
...
def score_pairs(pair_payloads: list[tuple[dict, np.ndarray, np.ndarray]], M: np.ndarray, ranges: np.ndarray, top_n: int | None, metric: str) -> list[float]:
    out: list[float] = []
...
def permutation_test(pair_payloads: list[tuple[dict, np.ndarray, np.ndarray]], M: np.ndarray, ranges: np.ndarray, n_perm: int, seed: int, top_n: int | None, metric: str) -> tuple[float, float, np.ndarray]:
...
def mann_whitney_auroc(scores: np.ndarray, labels: np.ndarray) -> tuple[float | None, float | None]:
...
def write_comparison_tsv(path: Path, v1: dict, v2_primary: dict) -> None:
    rows = [('metric', 'v1', 'v2'), ('n_pairs', v1.get('n_pairs'), v2_primary.get('n_pairs')), ('mean_pair_score_pearson', v1.get('mean_pair_score'), v2_primary.get('mean_pearson')), ('mean_pair_score_spearman', None, v2_primary.get('mean_spearman')), ('permutation_p_pearson', v1.get('p_value_right_tail'), v2_primary.get('p_value_pearson')), ('permutation_p_spearman', None, v2_primary.get('p_value_spearman')), ('significant_0_05', v1.get('significant_0_05'), v2_primary.get('significant_0_05_spearman')), ('mean_score_same_fold', v1.get('mean_score_same_fold'), v2_primary.get('mean_spearman_same_fold')), ('mean_score_diff_fold', v1.get('mean_score_diff_fold'), v2_primary.get('mean_spearman_diff_fold')), ('auroc_same_fold', None, v2_primary.get('auroc_same_fold')), ('mann_whitney_p', None, v2_primary.get('mann_whitney_p'))]
    with path.open('w') as f:
        for a, b, c in rows:
            f.write(f'{a}\t{b}\t{c}\n')

def analyze_cohort(records: list[dict], pair_payloads: list[tuple[dict, np.ndarray, np.ndarray]], M: np.ndarray, ranges: np.ndarray, cos_threshold: float, n_perm: int, seed: int, top_n: int | None) -> dict:
    ...
    _, p_pearson, null_p = permutation_test(sub_payloads, M, ranges, n_perm, seed, top_n, 'pearson')
    _, p_spearman, null_s = permutation_test(sub_payloads, M, ranges, n_perm, seed + 1, top_n, 'spearman')
    auc, mw_p = mann_whitney_auroc(spearmans, labels)
    same = spearmans[labels]
    diff = spearmans[~labels]
    return {'cohort': cohort_label(cos_threshold), 'feature_cosine_max': cos_threshold, 'n_pairs': len(sub_recs), 'mean_pearson': float(pearsons.mean()), 'median_pearson': float(np.median(pearsons)), 'mean_spearman': float(spearmans.mean()), 'median_spearman': float(np.median(spearmans)), 'p_value_pearson': p_pearson, 'p_value_spearman': p_spearman, 'null_mean_pearson': float(null_p.mean()), 'null_mean_spearman': float(null_s.mean()), 'significant_0_05_pearson': p_pearson < 0.05, 'significant_0_05_spearman': p_spearman < 0.05, 'mean_spearman_same_fold': float(same.mean()) if len(same) else None, 'mean_spearman_diff_fold': float(diff.mean()) if len(diff) else None, 'n_same_fold': int(labels.sum()), 'n_diff_fold': int((~labels).sum()), 'auroc_same_fold': auc, 'mann_whitney_p': mw_p}
...
def main() -> int:
    ...
    ap.add_argument('--top-dims', type=int, default=None, help='Ablation: keep top-N attributed dims (default: full from Exp A)')
    args = ap.parse_args()
    ...
    if not feat_table.exists():
        print(f'ERROR: missing {feat_table}. Run annotate_dssp.py --from-exp-a first.', file=sys.stderr)
        return 1
    ...
    if len(profiles_map) < 30:
        print(f'ERROR: only {len(profiles_map)} chains with binned profiles. Run: annotate_dssp.py --from-exp-a --also-train N --store-profiles', file=sys.stderr)
        return 1
    train_ids = [cid for cid, sp in splits.items() if sp == 'train' and cid in profiles_map and (cid in id_to_row)]
    train_ids.sort()
    ...
    np.savez_compressed(map_path, M=M, n_bins=n_bins, profile_features=np.array(bcfg.get('profile_features') or []), train_ids=np.array(train_ids), feature_ranges=ranges)
    top_dims = {}
    feat_labels = []
    ...
    write_json(results_dir / 'dim_profile_map.json', {'n_train': len(train_ids), 'dim': int(M.shape[0]), 'profile_dim': profile_dim, 'n_bins': n_bins, 'top_dims_per_profile_feature_sample': dict(list(top_dims.items())[:15]), 'max_abs_corr': float(np.abs(M).max()), 'mean_abs_corr': float(np.abs(M).mean())})
    top_n = args.top_dims
    ...
    print(f'Exp B v2 | train_map={len(train_ids)} profile_dim={profile_dim} planned={len(planned)} pending={len(pending)}')
    t0 = time.time()
    ...
    cohorts = [analyze_cohort(records, pair_payloads, M, ranges, t, n_perm, seed, top_n) for t in cohort_thresholds]
    ...
    for n_top in bcfg.get('ablation_top_dims') or []:
        if top_n is not None and n_top != top_n:
            continue
        ab = analyze_cohort(records, pair_payloads, M, ranges, 1.0, min(n_perm, 200), seed, int(n_top))
        ablations[f'top_{n_top}'] = {'mean_spearman': ab.get('mean_spearman'), 'p_value_spearman': ab.get('p_value_spearman'), 'significant_0_05_spearman': ab.get('significant_0_05_spearman')}
    v1_path = resolve_path(bcfg.get('exp_b_v1_metrics', 'results/exp_b/metrics.json'))
    v1 = load_v1_metrics(v1_path)
    metrics = {'version': 'exp_b_v2', 'mode': 'smoke' if smoke else 'paper', 'n_pairs': len(records), 'n_train_map': len(train_ids), 'profile_dim': profile_dim, 'n_bins': n_bins, 'top_dims_ablation': top_n, 'primary_cohort': primary, 'cohorts': cohorts, 'ablations': ablations, 'negative_control': {'saturated_diff_fold_cosine_min': sat_min, 'n_saturated_diff_fold': len(sat_recs), 'mean_spearman_saturated_diff_fold': float(np.mean(sat_scores)) if sat_scores else None, 'mean_spearman_same_fold': float(np.mean(same_scores)) if same_scores else None}, 'v1_reference': {'path': str(v1_path.relative_to(BIO_ROOT)) if v1_path.exists() else None, 'mean_pair_score': v1.get('mean_pair_score'), 'p_value_right_tail': v1.get('p_value_right_tail'), 'significant_0_05': v1.get('significant_0_05')}, 'n_permutations': n_perm, 'elapsed_s': round(time.time() - t0, 1), 'completed_unix': int(time.time())}
    ...
    write_comparison_tsv(results_dir / 'v1_vs_v2_comparison.tsv', v1, primary)
