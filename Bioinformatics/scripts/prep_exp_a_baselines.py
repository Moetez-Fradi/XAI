...
def main() -> int:
    ...
    stats = write_split_fastas(corpus_dir / 'sequences.fasta', corpus_dir / 'chains.csv', fasta_dir)
    ...
    pdb_dir = raw_dir('pdb')
    pending = [r for r in rows if args.restart or not (struct_dir / f"{r['chain_id']}.pdb").exists()]
    ...
    manifest = {**stats, 'structures': True, 'n_structures': n_struct, 'ok': ok, 'fail': fail, 'skip': skip, 'elapsed_s': round(time.time() - t0, 1), 'outdir': str(base.relative_to(BIO_ROOT))}
