...
def _find_cla(scop_dir: Path) -> Path:
    ...
    if not matches:
        raise FileNotFoundError(
            f"No SCOPe class file in {scop_dir}. Run download_scop.py first."
        )
    return matches[0]


def _parse_pdb_and_fold(parts: list[str]) -> tuple[str | None, str | None]:
    """Return (pdb_id, fold_key) from a SCOPe dir.cla line's tokens."""
    pdb_id = None
...
def sample_pdb_ids_stratified(
    ...
    *,
    ...
    """Unique PDB IDs, optionally round-robin across SCOPe folds."""
    ...
    # Round-robin across folds for fold coverage (paper held-out stratification)
...
def main() -> int:
    ...
    ap.add_argument(
        "--pilot",
        action="store_true",
        help="Small smoke set (pdb.pilot_unique_pdbs) instead of paper scale",
    )
    ...
    if args.ids_file:
        ...
        print(
            f"Sampling {limit} unique PDB IDs from {cla.name} "
            f"(mode={mode}, stratify={stratify}, seed={seed})"
        )
        ids = sample_pdb_ids_stratified(
            cla, limit, stratify=stratify, seed=seed
        )
    ...
    pilot_ids.write_text("\n".join(ids) + "\n")  # compat for older uniprot helper
        ...
        try:
            ...
            if fmt == "cif.gz":
                try:
                    ...
                    progress.tick(
                        ok=True,
                        nbytes=dest2.stat().st_size if dest2.exists() else 0,
                        force_print=(i == len(ids)),
                    )
                    recovered = True
    ...
    write_json(
        out / "download_manifest.json",
        {
            ...
            "failed": fail[:50],  # truncate huge lists
            "ids_file": str(paper_ids.relative_to(BIO_ROOT)),
            "outdir": str(out),
        },
    )
