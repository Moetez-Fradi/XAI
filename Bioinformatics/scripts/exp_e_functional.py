...
from common_bio import BatchProgress, ensure_dir, load_yaml, resolve_path, write_json
from exp_b_v2_attribution import append_jsonl, iter_exp_a, load_embeddings, load_jsonl, load_split, mann_whitney_auroc
from exp_e_lib import load_dim_map, load_dssp_residues, neighbor_pair_label, permutation_enrichment_null, resseq_site_mask, residue_attribution_scores, site_auc, site_enrichment, site_seq_indices_from_resseq, tmalign_mapping, tmalign_site_overlap, which_tmalign
...
def run_tmalign_batch(tasks: list[tuple[str, str, str, str]], struct_dir: Path, tm_bin: str, threads: int, ckpt: Path) -> dict[tuple[str, str], list[tuple[int, int]]]:
...
def main() -> int:
    ...
    corpus_dir = resolve_path(ecfg.get('corpus_smoke_dir' if smoke else 'corpus_dir', 'data/processed/corpus'))
    proc_dir = resolve_path(ecfg['processed_dir'])
    results_dir = resolve_path(ecfg['smoke_results_dir'] if smoke else ecfg['results_dir'])
    ...
    exp_a_jsonl = resolve_path(ecfg['exp_a_smoke_jsonl'] if smoke else ecfg['exp_a_jsonl'])
    ...
    neighbor_ec_go = build_neighbor_ec_go(meta, sifts_dir / 'pdb_chain_enzyme.csv', sifts_dir / 'pdb_chain_go.csv')
    ...
    for rec in iter_exp_a(exp_a_jsonl, exp_a_jsonl.parent.parent / 'per_query.json'):
        ...
        for nb in (rec.get('neighbors') or [])[:max_nb]:
            ...
            label = neighbor_pair_label(q_ec, q_go, q_fold, q_uni, n_ec, n_go, n_fold, n_uni, ec_level=ec_level, generic_go=generic_go)
            if label == 'unrelated':
                continue
            if label == 'fold_only' and (not include_fold_only):
                continue
            candidates.append({'query_chain_id': q, 'neighbor_chain_id': n, 'pair_label': label, 'contrib': nb.get('dims_explained') or {}, 'retrieval_score': float(nb.get('score') or 0), 'same_fold': fold_key(q_fold) == fold_key(n_fold)})
    ...
    print(f'Exp E | benchmark={len(benchmark)} candidate_pairs={len(candidates)} pending={len(pending)}')
    if not candidates:
        print('WARN: no candidate pairs — check Exp A `query` ids vs benchmark', file=sys.stderr)
    ...
    for i, c in enumerate(pending):
        ...
        rec: dict = {'query_chain_id': q, 'neighbor_chain_id': n, 'pair_label': c['pair_label'], 'retrieval_score': c['retrieval_score'], 'same_fold': c['same_fold'], 'n_site_residues': len(site_resseq)}
        contrib = c['contrib']
    ...
    if not args.skip_tmalign and struct_dir.exists():
        try:
            ...
            tm_bin = ''
        if tm_bin:
            max_tm = int(ecfg.get('tmalign_max_pairs_smoke' if smoke else 'tmalign_max_pairs', 150))
            if max_tm <= 0:
                max_tm = len(records)
            tm_tasks = []
            ...
            for r in records:
                ...
                if len(tm_tasks) >= max_tm:
                    break
            mappings = run_tmalign_batch(tm_tasks, struct_dir, tm_bin, int(ecfg.get('tmalign_threads', 4)), tm_ckpt)
            for (q, n), mapping in mappings.items():
                site_doc = sites[q]
                residues = load_dssp_residues(ann_dir, q)
                site_idx = site_seq_indices_from_resseq(residues, site_doc.get('site_resseq') or [])
                aln_q = {i - 1 for i, _ in mapping}
    ...
    if pooled_site_scores and pooled_bg_scores:
        site_labels = np.array([True] * len(pooled_site_scores) + [False] * len(pooled_bg_scores), dtype=bool)
    ...
    def _cohort_mw(rows_a: list[dict], rows_b: list[dict], key: str) -> tuple[float | None, float | None]:
        a = np.array([float(r[key]) for r in rows_a if r.get(key) is not None and np.isfinite(float(r[key]))], dtype=np.float64)
        b = np.array([float(r[key]) for r in rows_b if r.get(key) is not None and np.isfinite(float(r[key]))], dtype=np.float64)
    ...
    enrich_vals = np.array([float(r['xq_enrichment_ratio']) for r in func_rows if np.isfinite(float(r['xq_enrichment_ratio']))], dtype=np.float64)
    perm_p_gt_one = float('nan')
    perm_p_vs_one = float('nan')
    if len(enrich_vals) >= 3:
        from scipy.stats import binomtest, ttest_1samp
        n_gt = int(np.sum(enrich_vals > 1.0))
        perm_p_vs_one = float(binomtest(n_gt, len(enrich_vals), p=0.5, alternative='greater').pvalue)
    ...
    metrics = {'version': 'exp_e', 'mode': 'smoke' if smoke else 'paper', 'n_benchmark_queries': len(benchmark), 'n_pairs_total': len(records), 'n_pairs_functional_match': len(func_rows), 'n_pairs_fold_only': len(fold_rows), 'xq_site_enrichment': {'functional_match_mean': xq_func, 'fold_only_mean': xq_fold, 'functional_vs_fold_delta': xq_func - xq_fold if xq_func is not None and xq_fold is not None else None, 'mean_site_auc_functional': _mean(func_rows, 'xq_site_auc')}, 'tmalign_site_enrichment': {'functional_match_mean': tm_func, 'fold_only_mean': tm_fold, 'n_pairs_with_tmalign': sum((1 for r in records if r.get('tm_site_enrichment_ratio') is not None and np.isfinite(float(r['tm_site_enrichment_ratio']))))}, 'significance': {'mann_whitney_site_vs_background_auc': mw_auc, 'mann_whitney_site_vs_background_p': mw_p, 'mann_whitney_xq_functional_vs_fold_only_p': xq_cohort_p, 'mann_whitney_tm_functional_vs_fold_only_p': tm_cohort_p, 'perm_mean_enrichment_vs_shuffle_p': perm_p_gt_one, 'perm_mean_enrichment_gt_one_p': perm_p_vs_one, 'residue_shuffle_enrichment_p_example_pair': residue_perm_p}}
    write_json(results_dir / 'metrics.json', metrics)
    summary_rows = [['metric', 'value'], ['n_benchmark_queries', len(benchmark)], ['n_pairs_functional', len(func_rows)], ['n_pairs_fold_only', len(fold_rows)], ['xq_enrichment_functional_mean', xq_func], ['xq_enrichment_fold_only_mean', xq_fold], ['xq_site_auc_functional_mean', _mean(func_rows, 'xq_site_auc')], ['tm_enrichment_functional_mean', tm_func], ['tm_enrichment_fold_only_mean', tm_fold], ['mann_whitney_site_vs_bg_auc', mw_auc], ['mann_whitney_site_vs_bg_p', mw_p], ['xq_functional_vs_fold_only_p', xq_cohort_p], ['tm_functional_vs_fold_only_p', tm_cohort_p], ['perm_mean_enrichment_gt_one_p', perm_p_vs_one]]
    with (results_dir / 'summary.tsv').open('w', newline='') as f:
        w = csv.writer(f, delimiter='\t')
        w.writerows(summary_rows)
    sig_rows = [['test', 'statistic', 'p_value', 'notes'], ['mann_whitney_site_vs_background', mw_auc, mw_p, 'pooled functional-match pairs, per-residue attribution'], ['mann_whitney_xq_func_vs_fold', xq_cohort_auc, xq_cohort_p, 'pair-level xq_enrichment_ratio'], ['mann_whitney_tm_func_vs_fold', tm_cohort_auc, tm_cohort_p, 'pair-level tm_site_enrichment_ratio'], ['perm_mean_xq_enrichment_gt_one', _mean(func_rows, 'xq_enrichment_ratio'), perm_p_vs_one, 'null: enrichment<=1'], ['residue_shuffle_enrichment', 'ratio', residue_perm_p, 'example functional pair']]
    with (results_dir / 'significance.tsv').open('w', newline='') as f:
        w = csv.writer(f, delimiter='\t')
        w.writerows(sig_rows)
    loc_rows = [['query_chain_id', 'neighbor_chain_id', 'pair_label', 'xq_enrichment_ratio', 'xq_site_auc', 'tm_site_enrichment_ratio', 'tm_aligned_in_site_fraction']]
    for r in records:
        if r.get('error'):
            continue
        loc_rows.append([r['query_chain_id'], r['neighbor_chain_id'], r.get('pair_label'), r.get('xq_enrichment_ratio'), r.get('xq_site_auc'), r.get('tm_site_enrichment_ratio'), r.get('tm_aligned_in_site_fraction')])
    with (results_dir / 'localization_comparison.tsv').open('w', newline='') as f:
        w = csv.writer(f, delimiter='\t')
        w.writerows(loc_rows)
    write_json(results_dir / 'run_config.json', {'ecfg': ecfg, 'n_benchmark': len(benchmark), 'n_candidates': len(candidates), 'exp_a_jsonl': str(exp_a_jsonl)})
