...
from common_bio import BIO_ROOT, BatchProgress, download_cfg, download_file, ensure_dir, gunzip_to, load_yaml, raw_dir, resolve_path, write_json
UNIPROT_RE = re.compile('^(?:[OPQ][0-9][A-Z0-9]{3}[0-9]|[A-NR-Z][0-9][A-Z][A-Z0-9]{2}[0-9]|A0A[A-Z0-9]{2}[0-9]{6}(?:-\\d+)?|[A-Z0-9]{6,10})(?:\\.\\d+)?$', re.I)
...
def ensure_sifts(sifts_dir: Path, *, force: bool=False) -> Path:
    ...
    if csv_path.exists() and csv_path.stat().st_size > 0 and (not force):
        return csv_path
    url = download_cfg().get('uniprot', {}).get('sifts', [{}])[0].get('url', 'https://ftp.ebi.ac.uk/pub/databases/msd/sifts/flatfiles/csv/pdb_chain_uniprot.csv.gz')
...
def resolve_uniprot_id(raw: str, *, sleep: float=0.15) -> str | None:
    ...
    queries = [f'accession:{raw}', f'gene:{raw}', raw]
    for q in queries:
...
def collect_target_accessions(oma_csv: Path, seeds: list[str], meso_aliases: dict[str, str], *, include_all_thermo: bool) -> dict[str, dict]:
    ...
    if oma_csv.exists():
        with oma_csv.open() as f:
            for row in csv.DictReader(f):
                ...
                add(oid, {'roles': {'thermo_ortholog'}, 'seed_uniprot': seed, 'ortholog_species': sp, 'ortholog_species_name': sp_name, 'oma_id': oid})
                if include_all_thermo:
                    corpus_seed = meso_aliases.get(seed, seed)
                    add(corpus_seed, {'roles': {'meso_seed'}, 'seed_uniprot': seed})
    return targets

def rank_pdb_entries(entries: list[dict], *, max_n: int, corpus_pdbs: set[str], existing_files: set[str]) -> list[dict]:
...
def download_pdbs(pdb_ids: list[str], pdb_dir: Path, *, force: bool, sleep_s: float, retries: int) -> tuple[list[str], list[dict]]:
        ...
        try:
            ...
            if fmt == 'cif.gz':
                try:
                    ...
                    download_file(f'{base}/{pid}.cif', dest2, force=force, max_retries=retries, timeout=180, quiet=True)
                    ok.append(pdb_id.lower())
...
def main() -> int:
    ...
    max_per_u = int(args.max_pdbs_per_uniprot if args.max_pdbs_per_uniprot is not None else scfg.get('max_pdbs_per_uniprot', 3))
    ...
    targets = collect_target_accessions(oma_csv, seeds, aliases, include_all_thermo=bool(scfg.get('include_all_oma_thermophiles', True)))
    ...
    for acc, meta in sorted(targets.items()):
        ...
        if not entries:
            ...
            mapping_rows.append({'uniprot': acc, 'pdb_id': '', 'chain': '', 'source': '', 'roles': ','.join(sorted(meta.get('roles', []))), 'seed_uniprot': meta.get('seed_uniprot', ''), 'ortholog_species': meta.get('ortholog_species', ''), 'in_corpus_uniprot': acc in corpus_uniprots})
            continue
        picked = rank_pdb_entries(entries, max_n=max_per_u, corpus_pdbs=corpus_pdbs, existing_files=existing_files)
        if acc in corpus_uniprots:
            stats['already_in_corpus'] += 1
        for e in picked:
            pdb_ids_needed.add(e['pdb_id'])
            mapping_rows.append({'uniprot': acc, 'pdb_id': e['pdb_id'], 'chain': e['chain'], 'source': e['source'], 'roles': ','.join(sorted(meta.get('roles', []))), 'seed_uniprot': meta.get('seed_uniprot', ''), 'ortholog_species': meta.get('ortholog_species', ''), 'in_corpus_uniprot': acc in corpus_uniprots})
    ...
    with map_tsv.open('w', newline='') as f:
        fields = ['uniprot', 'pdb_id', 'chain', 'source', 'roles', 'seed_uniprot', 'ortholog_species', 'in_corpus_uniprot']
        w = csv.DictWriter(f, fieldnames=fields, delimiter='\t')
        w.writeheader()
        w.writerows(mapping_rows)
    ids_txt.write_text('\n'.join(pdb_list) + ('\n' if pdb_list else ''))
    print(f'Resolved {len(pdb_list)} unique PDB IDs → {ids_txt}')
    print(f"  sifts={stats['with_sifts']} rest={stats['with_rest']} no_structure={stats['no_structure']} acc_in_corpus={stats['already_in_corpus']}")
    ok, fail = ([], [])
    if not args.skip_download and pdb_list:
        print(f'Downloading {len(pdb_list)} structures → {pdb_dir}')
        ok, fail = download_pdbs(pdb_list, pdb_dir, force=args.force, sleep_s=float(scfg.get('sleep_s', 0.03)), retries=int(download_cfg()['pdb'].get('max_retries', 4)))
    manifest = {'n_target_uniprots': len(targets), 'n_mapping_rows': len(mapping_rows), 'n_pdb_ids': len(pdb_list), 'n_downloaded_ok': len(ok), 'n_download_failed': len(fail), 'stats': stats, 'max_pdbs_per_uniprot': max_per_u, 'mapping_tsv': str(map_tsv.relative_to(BIO_ROOT)), 'pdb_ids_file': str(ids_txt.relative_to(BIO_ROOT)), 'failures': fail[:30]}
