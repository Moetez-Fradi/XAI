...
from annotate_dssp import MAX_ASA, find_structure, materialize_structure, load_chain_targets
from common_bio import ensure_dir, resolve_path, write_json
NEG = frozenset('DE')
POS = frozenset('KRH')
...
def compute_chain_features(struct_path: Path, chain_id: str, *, ion_cutoff: float) -> dict:
    ...
    return {'ion_pair_count': int(n_pairs), 'ion_pair_density': float(ion_density), 'mean_sasa': float(mean_rsa), 'packing_density': float(packing), 'n_residues_struct': int(n_res), 'sasa_method': 'biopython_shrake_rupley'}
...
def ensure_structure_features(chain_ids: set[str], corpus_csv: Path, out_dir: Path, *, ion_cutoff: float=4.0, pdb_dir: Path | None=None) -> None:
    ensure_dir(out_dir)
