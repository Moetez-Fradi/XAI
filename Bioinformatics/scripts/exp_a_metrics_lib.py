...
def bootstrap_recall(flags: list[list[bool]], ks: list[int], *, n_boot: int=1000, seed: int=42) -> dict[str, dict[str, float]]:
    ...
    for k in ks:
        ...
        out[f'recall@{k}'] = {'mean': float(scores.mean()), 'ci_low': float(lo), 'ci_high': float(hi)}
    return out
...
def evaluate_rankings(query_ids: list[str], rankings: dict[str, list[str]], meta: dict[str, dict], ks: list[int], *, n_boot: int=1000, seed: int=42) -> dict:
