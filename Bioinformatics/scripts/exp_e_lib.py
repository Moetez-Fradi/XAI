...
from annotate_dssp import KD  # noqa: E402 — same package
from exp_b_v2_attribution import contrib_vector, fit_dim_feature_map, load_embeddings, load_split  # noqa: E402
...
def load_sifts_chain_uniprot(csv_path: Path) -> dict[tuple[str, str], list[dict]]:
    """Map (pdb, chain) -> list of SIFTS segment dicts."""
...
def parse_uniprot_feature_positions(entry: dict) -> list[dict[str, Any]]:
    """Extract functional feature intervals from UniProt REST JSON."""
...
def neighbor_pair_label(
    ...
    *,
    ...
    """Classify query–neighbor pair for Exp E."""
...
def ensure_benchmark_dssp(
    chain_ids: set[str],
    ann_dir: Path,
    mkdssp_cfg: str,
    corpus_csv: Path,
) -> None:
    """Ensure per-residue DSSP JSON exists for benchmark chains."""
...
def map_sites_for_chain(
    chain_id: str,
    uniprot_acc: str,
    uniprot_entry: dict,
    sifts_segments: list[dict],
    residues: list[dict],
    *,
    site_types: set[str],
    catalytic_keywords: list[str],
    buffer: int,
) -> dict[str, Any]:
    """Map UniProt functional features to PDB resseq on this chain."""
    ...
    # Sequence buffer around mapped sites
    ...
    return {
        ...
        "site_resseq": sorted(site_resseq),
...
def residue_feature_vector(r: dict, bin_idx: int, n_bins: int, n_feat: int = 5) -> np.ndarray:
    """One-hot-ish local feature vector aligned with binned profile columns."""
...
def residue_attribution_scores(
    residues: list[dict],
    contrib: dict[str, float],
    M: np.ndarray,
    n_bins: int = 32,
) -> np.ndarray:
    """Per-residue attribution proxy from dim→profile map."""
...
def site_enrichment(
    ...
    return {
        ...
        "site_fraction": float(site_mask.sum() / n),
    }
...
def which_tmalign() -> str:
    ...
    candidates = [
        BIO_ROOT / "tools/mamba/envs/bio-tools/bin/TMalign",
        BIO_ROOT / "tools/mamba/envs/bio-tools/bin/tmalign",
    ]
    for p in candidates:
    ...
    raise FileNotFoundError(
        "TMalign not found. Install via bio-tools conda env or ./fetch_tools_linux.sh"
    )


def parse_tmalign_stdout(stdout: str) -> list[tuple[int, int]]:
    """Parse TM-align (2024+) stdout sequence alignment block.

    Returns 1-based (query_idx, target_idx) pairs for structurally aligned residues.
    """
...
def tmalign_mapping(bin_path: str, q_pdb: Path, t_pdb: Path) -> list[tuple[int, int]]:
    """Return list of (query_seq_index, target_seq_index) 1-based."""
...
def tmalign_site_overlap(
    aligned_query_indices: set[int],
    site_seq_indices: set[int],
    n_res: int,
) -> dict[str, float]:
    """Fraction of aligned query positions in functional site vs uniform expectation."""
    n_aln = len(aligned_query_indices)
    n_site = len(site_seq_indices)
    if n_res <= 0 or n_aln == 0 or n_site == 0:
        return {
            "aligned_in_site_fraction": float("nan"),
            "expected_fraction": float("nan"),
            "enrichment_ratio": float("nan"),
            "n_aligned": float(n_aln),
        }
    ...
    return {
        "aligned_in_site_fraction": frac,
        "expected_fraction": expected,
        "enrichment_ratio": ratio,
        "n_aligned": float(n_aln),
        "n_overlap": float(overlap),
    }
...
def permutation_enrichment_null(
    scores: np.ndarray,
    site_mask: np.ndarray,
    n_perm: int,
    seed: int,
