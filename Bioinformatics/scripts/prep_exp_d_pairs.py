...
def resolve_corpus_dir(dcfg: dict, corpus_cfg: dict, smoke: bool) -> Path:
    if not smoke and dcfg.get('use_merged_corpus', True):
        ...
        if (merged / 'chains.csv').exists():
            return merged
    return resolve_path(corpus_cfg['smoke_outdir'] if smoke else corpus_cfg['paper_outdir'])
...
def _best_chain(cands: list[dict], prefer_holdout: bool=True) -> dict | None:
    if not cands:
        return None
    return sorted(cands, key=lambda r: (r['split'] != 'holdout' if prefer_holdout else False, r['chain'] not in {'A', 'B'}, r['chain_id']))[0]

def pairs_from_literature(path: Path, by_uniprot: dict[str, list[dict]], by_chain: dict) -> list[dict]:
    if not path.exists():
        return []
    out: list[dict] = []
    with path.open() as f:
        for row in csv.DictReader(f):
            ...
            if not meso and row.get('uniprot_meso'):
                meso = (_best_chain(by_uniprot.get(row['uniprot_meso'], [])) or {}).get('chain_id', '')
            if not thermo and row.get('uniprot_thermo'):
                thermo = (_best_chain(by_uniprot.get(row['uniprot_thermo'], [])) or {}).get('chain_id', '')
            if meso in by_chain and thermo in by_chain:
                out.append({'meso_chain_id': meso, 'thermo_chain_id': thermo, 'source': 'literature', 'note': row.get('note', '')})
    return out
...
def pairs_from_uniprot_map(map_tsv: Path, rows: list[dict], meso_aliases: dict[str, str], thermo_u: set[str]) -> list[dict]:
    ...
    for (seed, tu), sp in sorted(combos.items()):
        ...
        if not meso_cands or not thermo_cands:
            continue
        meso = sorted(meso_cands, key=lambda r: (r['split'] != 'holdout', r['chain'] not in {'A', 'B'}, r['chain_id']))[0]
        thermo = sorted([t for t in thermo_cands if t.get('uniprot') in thermo_u] or thermo_cands, key=lambda r: (r['chain'] != 'A', r['chain_id']))[0]
        key = (meso['chain_id'], thermo['chain_id'])
        if key in seen:
            continue
        seen.add(key)
        out.append({'meso_chain_id': meso['chain_id'], 'thermo_chain_id': thermo['chain_id'], 'source': 'structure_map', 'note': f'seed={seed} thermo={tu} species={sp}'})
    return out

def pairs_from_oma(oma_csv: Path, rows: list[dict], thermo_u: set[str], meso_aliases: dict[str, str]) -> list[dict]:
    ...
    with oma_csv.open() as f:
        for rec in csv.DictReader(f):
            ...
            try:
                ...
                normalize_uniprot = lambda x: (x or '').strip().split('.')[0].upper()
                resolve_uniprot_id = lambda x: None
            ...
            thermo = sorted([t for t in thermo_cands if t.get('uniprot') in thermo_u] or thermo_cands, key=lambda r: (r['chain'] != 'A', r['chain_id']))
            if not thermo:
                continue
            thermo = thermo[0]
            key = (meso['chain_id'], thermo['chain_id'])
            if key in seen:
                continue
            seen.add(key)
            out.append({'meso_chain_id': meso['chain_id'], 'thermo_chain_id': thermo['chain_id'], 'source': 'oma', 'note': f'seed={seed} ortholog_species={sp}'})
    return out

def pairs_auto_fold(rows: list[dict], thermo_u: set[str], thermo_pdb: set[str], min_len: int, max_pairs: int) -> list[dict]:
    thermo = [r for r in rows if is_thermo_row(r, thermo_u, thermo_pdb) and r['chain'] == 'A' and (int(r['length']) >= min_len)]
    meso = [r for r in rows if not is_thermo_row(r, thermo_u, thermo_pdb) and int(r['length']) >= min_len]
    ...
    for t in sorted(thermo_reps.values(), key=lambda r: r['chain_id']):
        ...
        if not mesos:
            continue
        mesos = sorted(mesos, key=lambda r: (r['split'] != 'holdout', abs(int(r['length']) - int(t['length']))))
        m = mesos[0]
        key = (m['chain_id'], t['chain_id'])
        if key in seen:
            continue
        seen.add(key)
        out.append({'meso_chain_id': m['chain_id'], 'thermo_chain_id': t['chain_id'], 'source': 'auto_fold', 'note': f'scop_fold={fold}'})
...
def main() -> int:
    ...
    def add_batch(batch: list[dict]) -> None:
        for p in batch:
            ...
            pairs.append(p)
    add_batch(pairs_from_literature(resolve_path(dcfg['literature_pairs_csv']), by_uniprot, by_chain))
    thermo_dir = resolve_path('data/raw/thermo')
    add_batch(pairs_from_uniprot_map(thermo_dir / 'exp_d_uniprot_pdbs.tsv', rows, meso_aliases, thermo_u))
    add_batch(pairs_from_oma(resolve_path(dcfg['oma_pairs_csv']), rows, thermo_u, meso_aliases))
    remaining = max(0, max_pairs - len(pairs))
    if remaining:
        add_batch(pairs_auto_fold(rows, thermo_u, thermo_pdb, int(dcfg.get('min_chain_length', 300)), remaining))
    pairs = pairs[:max_pairs]
    out_dir = ensure_dir(resolve_path(dcfg['pairs_out']).parent)
    out_csv = resolve_path(dcfg['pairs_out'])
    fields = ['meso_chain_id', 'thermo_chain_id', 'source', 'note', 'meso_split', 'thermo_split', 'meso_uniprot', 'thermo_uniprot', 'scop_fold_meso', 'scop_fold_thermo']
    with out_csv.open('w', newline='') as f:
        w = csv.DictWriter(f, fieldnames=fields, extrasaction='ignore')
        w.writeheader()
        w.writerows(pairs)
    write_json(out_dir / 'pairs_manifest.json', {'n_pairs': len(pairs), 'sources': {s: sum((1 for p in pairs if p['source'] == s)) for s in ('literature', 'structure_map', 'oma', 'auto_fold')}})
