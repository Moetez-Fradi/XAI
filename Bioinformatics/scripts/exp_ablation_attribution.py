...
from exp_b_v2_attribution import fit_dim_feature_map, load_profiles, load_split, pair_scores
...
def main() -> int:
    ...
    for cfg in configs:
        ...
        train_profiles = load_train_profiles_for_map(ann_dir, splits, id_to_row, n_bins, n_pf)
        ...
        try:
            ...
            mw_p = float('nan')
        rec = {'n_pairs': int(len(scores)), 'mean_spearman': float(arr.mean()), 'mean_spearman_same_fold': float(same.mean()) if len(same) else None, 'mean_spearman_diff_fold': float(diff.mean()) if len(diff) else None, 'fold_gap': float(same.mean() - diff.mean()) if len(same) and len(diff) else None, 'mann_whitney_p': float(mw_p), 'layer': cfg.get('layer'), 'pooling': cfg.get('pooling')}
    ...
    with (results_dir / 'ablation_comparison.tsv').open('w', newline='') as f:
        ...
        w.writerow(['config', 'layer', 'pooling', 'n_pairs', 'fold_gap', 'mann_whitney_p', 'mean_spearman'])
        for name, rec in sorted(all_metrics.get('configs', {}).items()):
            if rec.get('skipped'):
                continue
            w.writerow([name, rec.get('layer'), rec.get('pooling'), rec.get('n_pairs'), f"{rec.get('fold_gap', 0):.4f}", f"{rec.get('mann_whitney_p', 0):.4g}", f"{rec.get('mean_spearman', 0):.4f}"])
    return 0
