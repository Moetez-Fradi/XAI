...
def raw_dir(key: str) -> Path:
    cfg = paths_cfg()
    mapping = {'scop': cfg.get('scop_dir', 'data/raw/scop'), 'cath': cfg.get('cath_dir', 'data/raw/cath'), 'pdb': cfg.get('pdb_dir', 'data/raw/pdb'), 'uniprot': cfg.get('uniprot_dir', 'data/raw/uniprot'), 'pfam': cfg.get('pfam_dir', 'data/raw/pfam'), 'thermo': cfg.get('thermo_dir', 'data/raw/thermo'), 'sifts': cfg.get('sifts_dir', 'data/raw/sifts')}
...
class BatchProgress:
    ...
    def tick(self, *, ok: bool=True, skipped: bool=False, nbytes: int=0, force_print: bool=False, every_s: float=2.0) -> None:
        if skipped:
            self.skipped += 1
        elif ok:
            self.done += 1
    ...
    def print_status(self) -> None:
        elapsed = max(time.time() - self.t0, 1e-06)
        pct = 100.0 * self.completed / self.total
        rate = self.completed / elapsed
        remaining = max(self.total - self.completed, 0)
        eta = remaining / rate if rate > 0 else float('inf')
        bps = self.bytes_downloaded / elapsed
        print(f'  [{self.label}] {self.completed}/{self.total} ({pct:5.1f}%) | ok={self.done} skip={self.skipped} fail={self.failed} | {format_bytes(self.bytes_downloaded)} @ {format_bytes(bps)}/s | elapsed {format_duration(elapsed)} | ETA {format_duration(eta)}', flush=True)

def _download_with_curl(url: str, tmp: Path, *, timeout: int, max_retries: int, user_agent: str, quiet: bool, label: str) -> None:
    cmd = ['curl', '-L', '--fail', '--retry', str(max(max_retries, 5)), '--retry-delay', '5', '--retry-all-errors', '--connect-timeout', str(min(timeout, 60)), '--max-time', '0', '-A', user_agent, '-C', '-', '-o', str(tmp), url]
    if quiet:
        cmd.insert(1, '-sS')
    else:
...
def _download_with_urllib(url: str, tmp: Path, *, timeout: int, max_retries: int, user_agent: str, quiet: bool, label: str) -> None:
        ...
        try:
            ...
            with urlopen(req, timeout=timeout) as resp:
                ...
                if status == 206 and total_n is not None:
                    ...
                    expected_final = total_n if mode == 'wb' else existing + total_n if total_n is not None else None
                t0 = time.time()
                last_ui = 0.0
                with tmp.open(mode) as out:
                    while True:
                        ...
                        if not quiet and now - last_ui >= 0.5:
                            elapsed = max(now - t0, 1e-06)
                            speed = (got - (existing if mode == 'ab' else 0)) / elapsed
                            if expected_final:
                                pct = 100.0 * got / expected_final
                                eta = (expected_final - got) / speed if speed > 0 else float('inf')
                                msg = f'\r    {label}: {pct:5.1f}% {format_bytes(got)}/{format_bytes(expected_final)} @ {format_bytes(speed)}/s ETA {format_duration(eta)}   '
                            else:
                                msg = f'\r    {label}: {format_bytes(got)} @ {format_bytes(speed)}/s (size unknown)   '
                ...
                if expected_final is not None and final_size != expected_final:
                    raise OSError(f'incomplete download of {label}: got {final_size} bytes, expected {expected_final}')
            return
...
def download_file(url: str, dest: Path, *, force: bool=False, timeout: int=120, max_retries: int=3, user_agent: str='XQdrant-Bioinformatics/0.1 (research; +local)', quiet: bool=False, progress_label: str | None=None) -> Path:
    ...
    try:
        if shutil.which('curl'):
            _download_with_curl(url, tmp, timeout=timeout, max_retries=max_retries, user_agent=user_agent, quiet=quiet, label=label)
        else:
            _download_with_urllib(url, tmp, timeout=timeout, max_retries=max_retries, user_agent=user_agent, quiet=quiet, label=label)
    except Exception:
        raise
    tmp.replace(dest)
