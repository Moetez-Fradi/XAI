...
from common_bio import resolve_path  # noqa: E402
...
def build_arrays(max_neighbors: int):
    ...
    train_all = sorted(
        c for c, sp in splits.items() if sp == "train" and c in profiles and c in id2
    )
...
def main() -> int:
    ...
    for max_n in (5, 10):
        ...
        for name, m in (("corr", m_corr), ("ridge1", m_ridge)):
            ...
            print(
                f"max_n={max_n} map={name} pairs={len(scores)} train={n_train} "
                f"mean={scores.mean():.4f} sf={scores[labels].mean():.4f} "
                f"df={scores[~labels].mean():.4f} gap={gap:.4f}"
            )
    ...
    null_all = np.array(
        [pearson_rows(a @ m_corr[prng.permutation(m_corr.shape[0])], y).mean() for _ in range(5000)]
    )
    print(
        f"row_perm all: p={perm_p(float(scores.mean()), null_all):.4f} "
        f"real={scores.mean():.4f} null={null_all.mean():.4f}"
    )

    null_sf = np.array(
        [
            pearson_rows(a @ m_corr[prng.permutation(m_corr.shape[0])], y)[labels].mean()
            for _ in range(5000)
        ]
    )
    print(
        f"row_perm same_fold: p={perm_p(float(scores[labels].mean()), null_sf):.4f} "
        f"real={scores[labels].mean():.4f} null={null_sf.mean():.4f}"
    )

    real_gap = float(scores[labels].mean() - scores[~labels].mean())
    null_gap = np.array(
        [
            scores[prng.permutation(labels)].mean() - scores[~prng.permutation(labels)].mean()
            for _ in range(5000)
        ]
    )
...
def spearman_perm_from_v2() -> None:
    ...
    labels = np.array(
        [bool(fold_of(r["query"]) and fold_of(r["query"]) == fold_of(r["hit"])) for r in records]
    )
    ...
    y = np.stack(
        [profile_agreement(profiles[r["query"]], profiles[r["hit"]], ranges) for r in records]
    )
    ...
    nulls = np.array(
        [
            pearson_rows(rank_rows(a @ m[prng.permutation(m.shape[0])]), ry).mean()
            for _ in range(5000)
        ]
    )
    print(
        f"spearman global row_perm p={perm_p(real, nulls):.4f} "
        f"real={real:.4f} null={nulls.mean():.4f}"
    )

    real_sf = float(scores[labels].mean())
    nulls_sf = np.array(
        [
            pearson_rows(rank_rows(a @ m[prng.permutation(m.shape[0])]), ry)[labels].mean()
            for _ in range(5000)
        ]
    )
    print(
        f"spearman same_fold row_perm p={perm_p(real_sf, nulls_sf):.4f} "
        f"real={real_sf:.4f} null={nulls_sf.mean():.4f}"
    )
