...
def main() -> int:
    ...
    if not struct_dir.exists() or not any(struct_dir.glob('*.pdb')):
        print('ERROR: missing structures. Run prep_exp_a_baselines.py (without --skip-structures).', file=sys.stderr)
        return 1
    out_dir = ensure_dir(resolve_path(bcfg['smoke_results_dir'] if smoke else bcfg['results_dir']) / 'foldseek')
    ...
    if args.restart or not db_ready:
        ...
        subprocess.run([foldseek, 'createdb', str(train_dir), str(target_db)], check=True)
    if args.restart or not hits_path.exists():
        print('Running foldseek easy-search...')
        cmd = [foldseek, 'easy-search', str(hold_dir), str(target_db), str(hits_path), str(tmp_dir), '-s', str(fcfg.get('sensitivity', 9.5)), '--max-seqs', str(fcfg.get('max_seqs', 50)), '--threads', str(fcfg.get('threads', 4)), '--format-output', 'query,target,evalue,bits,alntmscore']
    ...
    hold_ids = sorted((p.stem for p in hold_dir.glob('*.pdb') if meta.get(p.stem, {}).get('split') == 'holdout'))
    topk = args.topk
