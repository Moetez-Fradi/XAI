...
from _common import RESULTS, ensure_out, save
...
def main() -> int:
    ...
    bp = axes[0].boxplot([d if d else [0] for d in data], tick_labels=labels, patch_artist=True, showfliers=False)
    ...
    def fmt_p(p) -> str:
        ...
        try:
            ...
            return str(p)
        if pv == 0.0 or pv < 1e-16:
            return '<1e-16'
        if pv < 0.001:
            return f'{pv:.1e}'
        return f'{pv:.3g}'
    fig.suptitle(f"Exp E | site vs bg MW p={fmt_p(sig.get('mann_whitney_site_vs_background_p'))} | XQ func mean={metrics.get('xq_site_enrichment', {}).get('functional_match_mean', '?'):.3f} | n=1550 func / 40 fold-only", fontsize=9)
    fig.tight_layout()
