...
from _common import ensure_out, save, setup_style

def box(ax, xy, w, h, text, fc='#dbeafe', ec='#2563eb'):
    rect = mpatches.FancyBboxPatch(xy, w, h, boxstyle='round,pad=0.02,rounding_size=0.02', fc=fc, ec=ec, lw=1.5)
    ax.add_patch(rect)
    ax.text(xy[0] + w / 2, xy[1] + h / 2, text, ha='center', va='center', fontsize=9, wrap=True)

def arrow(ax, x1, y1, x2, y2):
    ax.annotate('', xy=(x2, y2), xytext=(x1, y1), arrowprops=dict(arrowstyle='->', color='#475569', lw=1.5))
...
def main() -> int:
    ...
    ax.text(5.0, 2.55, 'XQdrant explainable protein retrieval pipeline', ha='center', fontsize=11, fontweight='bold')
