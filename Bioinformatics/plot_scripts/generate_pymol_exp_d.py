...
def main() -> int:
    ...
    lines = [
        ...
        f"# Pair: {meso} vs {thermo} | xq_spearman={pair.get('xq_profile_spearman')}",
        "reinitialize",
        "bg_color white",
        "set cartoon_fancy_helices, 1",
        "",
        f"load {meso_struct}, meso",
        f"remove meso and not chain {meso_ch}",
        "create meso_chain, meso",
        "delete meso",
        "",
        f"load {thermo_struct}, thermo",
        f"remove thermo and not chain {thermo_ch}",
        ...
        f"for obj, csvpath in [('meso_chain', r'{rel(meso_rsa)}'), ('thermo_chain', r'{rel(thermo_rsa)}')]:",
    ...
    lines2 = [
        ...
        "import csv",
        "from pymol import cmd",
        f"for obj, csvpath in [('meso_chain', r'{rel(meso_chg)}'), ('thermo_chain', r'{rel(thermo_chg)}')]:",
        "    with open(csvpath) as f:",
        "        for row in csv.DictReader(f):",
        "            resi = row['resi']",
        "            val = float(row['value'])",
