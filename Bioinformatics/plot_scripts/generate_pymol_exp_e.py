...
from _common import BIO_ROOT, PYMOL_OUT, RESULTS, ensure_out  # noqa: E402
...
from common_bio import load_yaml, resolve_path  # noqa: E402
from exp_b_v2_attribution import iter_exp_a  # noqa: E402
...
def load_tsv(path: Path) -> list[dict]:
    ...
    return [
        ...
        for parts in (ln.split("\t") for ln in lines[1:])
    ]
...
def main() -> int:
    ...
    lines = [
        f"# Exp E PyMOL — query {q} vs neighbor {n}",
        f"# xq_enrichment={best.get('xq_enrichment_ratio')}",
        "reinitialize",
        "bg_color white",
        f"load {struct}, prot",
        f"remove prot and not chain {chain}",
        "create query_chain, prot",
        "delete prot",
        "python",
        "import csv",
        "from pymol import cmd",
        f"for csvpath, col in [(r'{rel(attr_csv)}', 'b'), (r'{rel(site_csv)}', 'q')]:",
    ...
    # Matplotlib figure (paper-ready without PyMOL GUI)
    try:
        ...
        ax.set_ylim(0, max(scores) * 1.15 + 1e-6)
