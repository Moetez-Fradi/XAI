...
from _common import OUTDIR, RESULTS, ensure_out, save, setup_style  # noqa: E402
...
def main() -> int:
    ...
    configs = [
        (k, v)
        for k, v in sorted(data.get("configs", {}).items())
        if not v.get("skipped") and v.get("fold_gap") is not None
    ]
    if not configs:
