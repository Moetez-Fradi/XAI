...
from _common import OUTDIR, PALETTE, RESULTS, ensure_out, save, setup_style
...
def main() -> int:
    ...
    ax.bar(x, manual, width=w, bottom=auto, color='#94a3b8', hatch='//', edgecolor='white', label=f'Manual PyMOL (est. {pymol_s / 60:.0f} min)')
    ...
    ax2.axis('off')
    rows = [['Workflow', 'Auto steps', 'Manual', 'Median auto (s)', 'Median total (s)', 'Rationale'], ['XQdrant (cold)', str(xq['automated_steps']), '0', f"{xq['median_s']:.2f}", f"{xq['median_s']:.2f}", 'embed + dims_explained']]
    if indexed:
        rows.append(['XQdrant (indexed)', '1', '0', f"{indexed['xqdrant']['median_s']:.3f}", f"{indexed['xqdrant']['median_s']:.3f}", 'search + dims_explained'])
    rows.append(['Foldseek→TM→PyMOL', '2', '1', f"{fs['automated_only']['median_s']:.2f}", f"{fs['with_manual_pymol']['median_s']:.0f}", 'visual inspection'])
    if bl is not None:
        rows.append(['BLAST→TM→PyMOL', '2', '1', f"{bl['automated_only']['median_s']:.2f}", f"{bl['with_manual_pymol']['median_s']:.0f}", 'visual inspection'])
