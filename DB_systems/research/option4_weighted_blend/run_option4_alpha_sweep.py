...
This script sweeps ``alpha x mask_from_layer`` and reports recall + speedup.
...
DEFAULT_LAYERS = [0]  # pair with Option 2 cutoffs; default fully masked
    ...
    for dim in args.dimensions:
        ...
        for layer in args.layers:
            for alpha in args.alphas:
                ...
                rows.append({
                    "dimension": dim, "subspace_ratio": args.ratio, "mask_from_layer": layer,
                    "alpha": alpha, "ef_search": args.ef_search, "k": args.k,
                ...
                print(f"  D={dim} layer={layer} alpha={alpha}: recall={recall:.3f}{tag}"
        ...
        series = {}
        for layer in args.layers:
            lr = [r for r in rows if r["mask_from_layer"] == layer]
            series[f"recall (mask_from_layer={layer})"] = [r["recall_at_k"] for r in lr]
        cr.line_plot(
            args.alphas, series,
            xlabel="alpha (0 = focus only, 1 = full distance)", ylabel="Recall@K (subspace GT)",
            title=f"{prefix}Option 4 blend recall (D={dim}, ratio={args.ratio})",
            stem=f"option4_alpha_recall_d{dim}", hline=None,
        )
