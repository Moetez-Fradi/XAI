    ...
    for dataset in cr.iter_datasets(args):
        ...
        for ratio in args.ratios:
            ...
            if args.mode == "http":
                ...
                recalls: list[float] = []
                m2_lat = LatencyStats()
                m2_recalls: list[float] = []
                client = cr.ResearchClient(
                    dataset, args.xqdrant_url, args.xqdrant_url, provision=False,
                    collection_name=name,
                )
                # M2 comparison lives on the original full-space collection if already provisioned.
                full_client = cr.ResearchClient(
                    dataset, args.xqdrant_url, args.xqdrant_url, provision=True,
                    collection_name=f"ceiling_full_{dataset.name}",
                )
                for q in queries:
                    ...
                    m2_out = full_client.query(cr.focus_body(
                        q, args.k, args.ef_search, dims, masked=True, mask_from_layer=1
                    ))
                    if m2_out.status == "ok":
                        m2_lat.record(m2_out.latency_ns)
                        m2_recalls.append(cr.recall_at_k(m2_out.result.ids, gt_ids, args.k))
                try:
                ...
                m2_recall = float(np.mean(m2_recalls)) if m2_recalls else float("nan")
                m2_p50 = m2_lat.percentiles_ms()["p50"] if m2_recalls else float("nan")
            else:
                ...
                m2_recall = float(np.clip(0.55 * ratio + 0.20, 0, 1))
                m2_p50 = 1.4
            ...
            rows.append({
                ...
                "m2_recall_at_k": m2_recall, "m2_p50_ms": m2_p50,
        ...
        cr.line_plot(
            [r["subspace_ratio"] for r in rows],
            {
                ...
                "M2 on full-space HNSW": [r["m2_recall_at_k"] for r in rows],
            },
            xlabel="Subspace ratio (D_sub / D)", ylabel="Recall@K (subspace GT)",
            title=f"{prefix}Projected ceiling vs M2 (D={dim})",
            stem="fig22_projected_ceiling", hline=1.0,
        )
        cr.line_plot(
            [r["subspace_ratio"] for r in rows],
