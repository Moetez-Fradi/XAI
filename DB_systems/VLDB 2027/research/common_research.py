...
def contiguous_indices(dimension: int, ratio: float) -> np.ndarray:
    """Leading contiguous block of coordinates (higher coherence than random)."""
    count = max(1, int(round(dimension * ratio)))
    return np.arange(count, dtype=np.int64)


def top_energy_indices(query: np.ndarray, ratio: float) -> np.ndarray:
    """Focus on coordinates with largest |q_i| (query-adaptive)."""
    count = max(1, int(round(len(query) * ratio)))
    return np.sort(np.argpartition(np.abs(query), -count)[-count:].astype(np.int64))
...
def add_dataset_args(parser) -> None:
    """Add --dataset / corpus dirs / --num-vectors to a research script's argparser."""
    parser.add_argument(
        "--dataset",
        choices=["synthetic", "sift1m", "gist1m", "minilm"],
        default="synthetic",
        help="Corpus: synthetic (Dot), sift1m (Euclid D=128), gist1m (Euclid D=960), minilm (Cosine D=384)",
    )
    ...
    parser.add_argument("--gist-dir", default=str(GIST_DIR),
                        help="Directory with gist_base.fvecs / gist_query.fvecs")
    parser.add_argument("--minilm-dir", default=str(MINILM_DIR),
                        help="Directory with embeddings.npy / queries.npy from fetch_minilm.sh")
    parser.add_argument("--num-vectors", type=int, default=None,
                        help="Cap corpus size (subsample SIFT/GIST/MiniLM for quick runs)")


def iter_datasets(args):
    """
    Yield the dataset(s) for a run. Real corpora are one fixed-D corpus each;
    synthetic sweeps ``args.dimensions``.
    """
    ...
    if kind == "sift1m":
        ...
        yield build_dataset("gist1m", gist_dir=getattr(args, "gist_dir", str(GIST_DIR)),
                            num_vectors=num_vectors, num_queries=num_queries)
    elif kind == "minilm":
        yield build_dataset("minilm", minilm_dir=getattr(args, "minilm_dir", str(MINILM_DIR)),
                            num_vectors=num_vectors, num_queries=num_queries)
    else:
...
def payload_filter(selectivity: float, n_buckets: int = 10) -> dict[str, Any] | None:
    """
    Qdrant filter matching a fraction ``selectivity`` of points tagged with
    ``payload.bucket in {0 .. n_buckets-1}`` at upsert time.
    ``selectivity=1.0`` returns None (unfiltered).
    """
    if selectivity >= 0.999:
        return None
    keep = max(1, int(round(selectivity * n_buckets)))
    return {
        "must": [
            {"key": "bucket", "match": {"any": list(range(keep))}}
        ]
    }


def attach_filter(body: dict[str, Any], filt: dict[str, Any] | None) -> dict[str, Any]:
    if filt is not None:
        body = dict(body)
        body["filter"] = filt
    return body
...
class ResearchClient:
    ...
    def __init__(
        ...
        with_payload_buckets: bool = False,
        ...
        self._backend = HttpBackend(
            ...
            with_payload_buckets=with_payload_buckets,
