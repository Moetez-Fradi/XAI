...
GIST_DIR = Path(os.environ.get("GIST_DIR", str(PACKAGE_DIR / "data" / "gist")))
MINILM_DIR = Path(os.environ.get("MINILM_DIR", str(PACKAGE_DIR / "data" / "minilm")))
PAPER_FIGURES_DIR = PACKAGE_DIR / "paper" / "figures"
...
def load_gist1m(
    data_dir: Path | str = GIST_DIR,
    num_vectors: int | None = None,
    num_queries: int | None = None,
) -> Dataset:
    """
    Load GIST1M (Euclidean, D=960) from TEXMEX ``gist_*.fvecs`` files.

    Bundled ground truth is only attached when the base is not subsampled.
    """
    data_dir = Path(data_dir)
    base_path = data_dir / "gist_base.fvecs"
    query_path = data_dir / "gist_query.fvecs"
    gt_path = data_dir / "gist_groundtruth.ivecs"
    if not base_path.exists():
        raise FileNotFoundError(
            f"GIST base vectors not found at {base_path}. "
            f"Download with fetch_gist.sh (see README)."
        )
    vectors = read_fvecs(base_path, limit=num_vectors)
    queries = read_fvecs(query_path, limit=num_queries)
    dimension = int(vectors.shape[1])
    ...
    ids = np.arange(vectors.shape[0], dtype=np.int64)
    return Dataset(
        name=f"gist1m_n{vectors.shape[0]}",
        dimension=dimension,
        vectors=vectors,
        queries=queries,
        ids=ids,
...
def load_minilm(
    data_dir: Path | str = MINILM_DIR,
    num_vectors: int | None = None,
    num_queries: int | None = None,
) -> Dataset:
    """
    Load cached MS MARCO / all-MiniLM-L6-v2 embeddings (D=384, Cosine).

    Expects ``embeddings.npy`` (N, 384) and ``queries.npy`` (Q, 384) written by
    ``fetch_minilm.sh``. Optional ``query_texts.jsonl`` is used by the case study.
    """
    data_dir = Path(data_dir)
    vec_path = data_dir / "embeddings.npy"
    q_path = data_dir / "queries.npy"
    if not vec_path.exists():
        raise FileNotFoundError(
            f"MiniLM embeddings not found at {vec_path}. "
            f"Build with fetch_minilm.sh (see README)."
        )
    vectors = np.load(vec_path).astype(np.float32, copy=False)
    if num_vectors is not None:
        vectors = vectors[:num_vectors]
    if q_path.exists():
        queries = np.load(q_path).astype(np.float32, copy=False)
        if num_queries is not None:
            queries = queries[:num_queries]
    else:
        nq = num_queries or min(NUM_QUERIES, vectors.shape[0])
        queries = vectors[:nq].copy()
    ids = np.arange(vectors.shape[0], dtype=np.int64)
    return Dataset(
        name=f"minilm_n{vectors.shape[0]}",
        dimension=int(vectors.shape[1]),
...
def build_dataset(
    ...
    *,
    ...
    gist_dir: Path | str = GIST_DIR,
    minilm_dir: Path | str = MINILM_DIR,
    num_vectors: int | None = None,
    num_queries: int | None = None,
    seed: int = RANDOM_SEED,
) -> Dataset:
    """Factory: synthetic / sift1m / gist1m / minilm."""
    kind = kind.lower()
    ...
    if kind in ("gist", "gist1m"):
        return load_gist1m(gist_dir, num_vectors=num_vectors, num_queries=num_queries)
    if kind in ("minilm", "marco", "msmarco"):
        return load_minilm(minilm_dir, num_vectors=num_vectors, num_queries=num_queries)
    raise ValueError(
        f"Unknown dataset kind: {kind!r}. Use 'synthetic', 'sift1m', 'gist1m', or 'minilm'."
    )
