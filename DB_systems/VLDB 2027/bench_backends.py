...
class HttpBackend(SearchBackend):
    ...
    def __init__(
        ...
        self.with_payload_buckets = with_payload_buckets
        self.n_payload_buckets = n_payload_buckets
    ...
    def _provision_collection(self) -> None:
        ...
        for base in {self.qdrant_url, self.xqdrant_url}:
            ...
            if self.quantization:
                # Scalar quantization (int8). Masked focus is rejected by XQdrant
                # on SQ collections (fail-fast) — that 400 is a measured result.
                qname = self.quantization.lower()
                if qname in ("sq", "scalar", "int8"):
                    create_body["quantization_config"] = {
                        "scalar": {"type": "int8", "quantile": 0.99, "always_ram": True}
                    }
                else:
                    raise ValueError(f"Unknown quantization mode: {self.quantization!r}")
            ...
            batch_size = 256
            n_buck = max(1, int(self.n_payload_buckets))
            for start in range(0, self.dataset.num_vectors, batch_size):
                ...
                for i in range(start, end):
                    ...
                    if self.with_payload_buckets:
                        pt["payload"] = {"bucket": int(self.dataset.ids[i]) % n_buck}
                    points.append(pt)
